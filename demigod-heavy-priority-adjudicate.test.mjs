...
const CLI = path.join(OPS, 'demigod-heavy-priority-adjudicate.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/priority-adjudicate';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'priority-adjudicate');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-PRIORITY.json',
  '/home/potter/HEAVY-PRIORITY-ADJUDICATE.md',
  '/home/potter/DEMIGOD-PRIORITY-CURSOR-PROMPT.md',
  '/home/potter/HEAVY-PRIORITY-ADJUDICATE-SENT.txt',
  path.join(OPS, 'DEMIGOD-HEAVY-PRIORITY.json'),
  path.join(OPS, 'HEAVY-PRIORITY-ADJUDICATE.md'),
  path.join(OPS, 'DEMIGOD-PRIORITY-CURSOR-PROMPT.md'),
  path.join(OPS, 'HEAVY-PRIORITY-ADJUDICATE-SENT.txt'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-priority') || name.startsWith('DEMIGOD-HEAVY-PRIORITY') || name.startsWith('HEAVY-PRIORITY-ADJUDICATE') || name.startsWith('DEMIGOD-PRIORITY-CURSOR')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-PRIORITY.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-PRIORITY-ADJUDICATE-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'PRIORITY notes stay local.',
    'ADJUDICATE stays on disk.',
    'RANK stays planted.',
    'BOARD stays local.',
...
describe('priority adjudicate', { concurrency: 1 }, () => {
  test('a priority record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-pri-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-pri-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-pri-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-pri-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-pri-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-pri-west-'));
    ...
    assert.equal(unsetOut.error, 'priority_root_required');
    ...
    assert.equal(homeOut.error, 'priority_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-PRIORITY.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-PRIORITY-ADJUDICATE.md'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-PRIORITY-ADJUDICATE-SENT.txt'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'priority_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-PRIORITY-ADJUDICATE.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-PRIORITY-ADJUDICATE-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'DEMIGOD-PRIORITY-CURSOR-PROMPT.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-PRIORITY-ADJUDICATE.md')), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-PRIORITY-ADJUDICATE-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(east, 'DEMIGOD-PRIORITY-CURSOR-PROMPT.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-PRIORITY-ADJUDICATE-SOURCE.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'PRIORITY notes only.');
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-PRIORITY-ADJUDICATE.md')), false);
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-PRIORITY-ADJUDICATE-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(bad, 'DEMIGOD-PRIORITY-CURSOR-PROMPT.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('ADJUDICATE'), true);
    assert.equal(badStored.missing.includes('RANK'), true);
    ...
    assert.equal(eastRun.status, 0, `east priority failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.PRIORITY, true);
    assert.equal(stored.markers.ADJUDICATE, true);
    assert.equal(stored.markers.RANK, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'priority-adjudicate', 'priority.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west priority failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-PRIORITY-ADJUDICATE-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'DEMIGOD-PRIORITY-CURSOR-PROMPT.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'priority-adjudicate', 'priority.shot')), false);
