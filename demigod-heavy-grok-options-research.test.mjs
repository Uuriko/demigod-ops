...
const CLI = path.join(OPS, 'demigod-heavy-grok-options-research.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/grok-build-options';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'grok-build-options');
const SHARED = [
  '/home/potter/DEMIGOD-GROK-BUILD-OPTIONS-RESEARCH.json',
  '/home/potter/HEAVY-GROK-BUILD-OPTIONS-RESEARCH.md',
  '/home/potter/HEAVY-GROK-BUILD-OPTIONS-SENT.txt',
  path.join(OPS, 'DEMIGOD-GROK-BUILD-OPTIONS-RESEARCH.json'),
  path.join(OPS, 'HEAVY-GROK-BUILD-OPTIONS-RESEARCH.md'),
  path.join(OPS, 'HEAVY-GROK-BUILD-OPTIONS-SENT.txt'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-grok-options') || name.startsWith('HEAVY-GROK-BUILD-OPTIONS')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-GROK-BUILD-OPTIONS-RESEARCH.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-GROK-BUILD-OPTIONS-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'OPTION notes stay local.',
    'ROUTING stays on disk.',
    'RULE split stays planted.',
    'PROJECT split stays local.',
    'BUILD work stays local.',
  ].join('\n');
}

describe('grok build options', { concurrency: 1 }, () => {
  test('an options record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-gopt-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-gopt-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-gopt-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-gopt-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-gopt-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-gopt-west-'));
    ...
    assert.equal(unsetOut.error, 'options_root_required');
    ...
    assert.equal(homeOut.error, 'options_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-GROK-BUILD-OPTIONS-RESEARCH.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-GROK-BUILD-OPTIONS-RESEARCH.md'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-GROK-BUILD-OPTIONS-SENT.txt'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'options_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-GROK-BUILD-OPTIONS-RESEARCH.md')), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-GROK-BUILD-OPTIONS-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-GROK-BUILD-OPTIONS-SOURCE.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'OPTION notes only.');
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-GROK-BUILD-OPTIONS-RESEARCH.md')), false);
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-GROK-BUILD-OPTIONS-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('ROUTING'), true);
    ...
    assert.equal(eastRun.status, 0, `east options failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.OPTION, true);
    assert.equal(stored.markers.ROUTING, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'grok-build-options', 'options.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west options failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-GROK-BUILD-OPTIONS-RESEARCH.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-GROK-BUILD-OPTIONS-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'grok-build-options', 'options.shot')), false);
