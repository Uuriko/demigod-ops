#!/usr/bin/env node
/**
 * Local custom code check in an explicit data root.
 * Writes DEMIGOD-FIX-CUSTOM-CODE.json under DEMIGOD_ROOT. Does not open a browser or send a prompt.
 */
import fs from 'fs';
...
function reportPath() {
  return path.join(dataRoot(), 'DEMIGOD-FIX-CUSTOM-CODE.json');
}
function shotDir() {
  return path.join(dataRoot(), 'audit-shots', 'fix-custom-code');
}
...
function main() {
  ...
  if (!root || path.resolve(root) === '/home/potter' || path.resolve(root) === path.resolve(scriptDir())) {
    refuse('code_root_required');
  }
  const footText = readText(root, 'demigod-foot-core.js');
  const notes = readText(root, 'HEAVY-FIX-CUSTOM-CODE-SOURCE.md');
  ...
  const dir = shotDir();
  const shot = path.join(dir, 'code.shot');
