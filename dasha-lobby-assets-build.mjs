...
function build() {
  ...
  const graphPage = readFileSync(join(root, 'dasha-graph-page.html'), 'utf8');
  ...
  const danceGlb = readFileSync(join(root, 'dasha-worker-assets/client/dasha.glb'));
  ...
  return {
    ...
    graphPageBytes: graphPage.length,
...
if (args.has('--check')) {
  ...
  if (!graphPageCheck.includes(`integrity="${built.graphSri}"`) || !graphPageCheck.includes('crossorigin="anonymous"')) {
    console.error('dasha-lobby-assets-build: graph page client SRI is stale');
    process.exit(1);
  }
