...
function build() {
  ...
  const loginPage = readFileSync(join(root, 'dasha-login-page.html'), 'utf8')
  ...
  const faucetPage = readFileSync(join(root, 'dasha-faucet-page.html'), 'utf8')
    ...
    .replace(/__X_CONNECT_SRI__/g, xConnectSri);
  const studioManifest = readFileSync(join(root, 'dasha-studio.webmanifest'), 'utf8').trim() + '\n';
  const studioIcon192 = readFileSync(join(root, 'dasha-worker-assets/client/dasha-icon-192.png'));
  const studioIcon512 = readFileSync(join(root, 'dasha-worker-assets/client/dasha-icon-512.png'));
  ...
  const hash = createHash('sha256')
    .update(
      worker +
        ...
        studioManifest +
        '\n' +
        studioIcon192.toString('base64') +
        '\n' +
        studioIcon512.toString('base64') +
        '\n' +
  ...
  return {
    ...
    loginPageBytes: loginPage.length,
...
export const LOGIN_PAGE_HTML = \`${escTemplate(loginPage)}\`;
export const FAUCET_PAGE_HTML = \`${escTemplate(faucetPage)}\`;
export const STUDIO_WEBMANIFEST = \`${escTemplate(studioManifest)}\`;
...
if (args.has('--check')) {
  ...
  const landingPinned = !landing.includes('client/simp-board.js') ||
  ...
  if (!landingPinned || (landing.includes('client/simp-board.js') && !landing.includes("s.crossOrigin='anonymous'"))) {
...
if (args.has('--write') || args.size === 0) {
  ...
  {
    const landing = readFileSync(join(root, 'dasha-landing.html'), 'utf8');
    if (!landing.includes('client/simp-board.js')) {
      // Simp has its own edge page; home no longer loads the board client.
    } else if (/const SIMP_SRI='sha384-[A-Za-z0-9+/=]+'/.test(landing)) {
