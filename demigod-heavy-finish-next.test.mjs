...
const CLI = path.join(OPS, 'demigod-heavy-finish-next.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/finish-next';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'finish-next');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-FINISH-NEXT.json',
  '/home/potter/HEAVY-CURSOR-FINISH-NEXT-PROMPT.md',
  '/home/potter/HEAVY-CURSOR-FINISH-NEXT-SENT.txt',
  path.join(OPS, 'DEMIGOD-HEAVY-FINISH-NEXT.json'),
  path.join(OPS, 'HEAVY-CURSOR-FINISH-NEXT-PROMPT.md'),
  path.join(OPS, 'HEAVY-CURSOR-FINISH-NEXT-SENT.txt'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-finish-next') || name.startsWith('HEAVY-CURSOR-FINISH-NEXT')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-FINISH-NEXT.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-CURSOR-FINISH-NEXT-BRIEF.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'FINISH notes stay local.',
    'NEXT work stays on disk.',
    'PROMPT stays unsent.',
    'LOCAL signals only.',
    'FORM counts stay planted.',
  ].join('\n');
}

describe('finish next', { concurrency: 1 }, () => {
  test('a finish-next record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-finish-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-finish-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-finish-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-finish-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-finish-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-finish-west-'));
    ...
    assert.equal(src.includes('scanLiveHtml'), false);
    ...
    assert.equal(unsetOut.error, 'finish_root_required');
    ...
    assert.equal(homeOut.error, 'finish_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-FINISH-NEXT.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-CURSOR-FINISH-NEXT-PROMPT.md'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-CURSOR-FINISH-NEXT-SENT.txt'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'finish_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const refused = run(east, ['--fetch'], {}, cwdDir);
    const refusedOut = readJson(refused, 'fetch');
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-CURSOR-FINISH-NEXT-PROMPT.md')), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-CURSOR-FINISH-NEXT-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-CURSOR-FINISH-NEXT-BRIEF.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'FINISH notes only.');
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-CURSOR-FINISH-NEXT-PROMPT.md')), false);
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-CURSOR-FINISH-NEXT-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('PROMPT'), true);
    ...
    assert.equal(stored.markers.FINISH, true);
    assert.equal(stored.markers.NEXT, true);
    assert.equal(stored.markers.PROMPT, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'finish-next', 'finish.shot'), 'utf8');
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-CURSOR-FINISH-NEXT-PROMPT.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-CURSOR-FINISH-NEXT-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'finish-next', 'finish.shot')), false);
