...
const CLI = path.join(OPS, 'demigod-heavy-competitors-followup.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/competitors-followup';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'competitors-followup');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-COMPETITORS-FOLLOWUP.json',
  '/home/potter/HEAVY-COMPETITORS-FOLLOWUP-SENT.txt',
  '/home/potter/HEAVY-COMPETITORS-FEATURES-REPLY-PART2.md',
  path.join(OPS, 'DEMIGOD-HEAVY-COMPETITORS-FOLLOWUP.json'),
  path.join(OPS, 'HEAVY-COMPETITORS-FOLLOWUP-SENT.txt'),
  path.join(OPS, 'HEAVY-COMPETITORS-FEATURES-REPLY-PART2.md'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-competitors-') || name.startsWith('HEAVY-COMPETITORS-')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-COMPETITORS-FOLLOWUP.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-COMPETITORS-FOLLOWUP-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'COMPETITOR notes stay local.',
    'TAXONOMY of trust and intake.',
    'WEDGE is the human match.',
    'BACKLOG fits the foot.',
    'COPY counters the other headline.',
    'VERDICT is a proof loop.',
  ].join('\n');
}

describe('competitors followup', { concurrency: 1 }, () => {
  test('a followup stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-follow-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-follow-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-follow-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-follow-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-follow-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-follow-west-'));
    ...
    assert.equal(src.includes('findGrokPage'), false);
    ...
    assert.equal(unsetOut.error, 'followup_root_required');
    ...
    assert.equal(homeOut.error, 'followup_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-COMPETITORS-FOLLOWUP.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-COMPETITORS-FOLLOWUP-SENT.txt'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'followup_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-COMPETITORS-FOLLOWUP-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-COMPETITORS-FOLLOWUP-SOURCE.md'));
    const linked = run(leak, [], {}, cwdDir);
    const linkedOut = readJson(linked, 'symlink note');
    ...
    assert.equal(badRun.status, 1, `thin note should fail\n${badRun.stdout}\n${badRun.stderr}`);
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-COMPETITORS-FOLLOWUP-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-COMPETITORS-FEATURES-REPLY-PART2.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('VERDICT'), true);

    plant(east, 'Harbor East keep', goodBody());
    const eastRun = run(east, [], {}, cwdDir);
    assert.equal(eastRun.status, 0, `east followup failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-COMPETITORS-FOLLOWUP-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-COMPETITORS-FEATURES-REPLY-PART2.md')), false);
    ...
    assert.equal(stored.sections.COMPETITOR, true);
    assert.equal(stored.sections.TAXONOMY, true);
    assert.equal(stored.sections.WEDGE, true);
    assert.equal(stored.sections.BACKLOG, true);
    assert.equal(stored.sections.COPY, true);
    assert.equal(stored.sections.VERDICT, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'competitors-followup', 'followup.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west followup failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-COMPETITORS-FOLLOWUP-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'competitors-followup', 'followup.shot')), false);
