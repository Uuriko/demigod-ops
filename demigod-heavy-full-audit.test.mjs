...
const CLI = path.join(OPS, 'demigod-heavy-full-audit.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/heavy-full-audit';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'heavy-full-audit');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-FULL-AUDIT.json',
  '/home/potter/HEAVY-FULL-AUDIT-REPLY.md',
  '/home/potter/DEMIGOD-FULL-AUDIT.json',
  path.join(OPS, 'DEMIGOD-HEAVY-FULL-AUDIT.json'),
  path.join(OPS, 'HEAVY-FULL-AUDIT-REPLY.md'),
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-full-audit') || name.startsWith('HEAVY-FULL-AUDIT')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-FULL-AUDIT.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-FULL-AUDIT-BRIEF.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'AUDIT notes stay local.',
    'FORM counts stay planted.',
    'META drift stays on disk.',
    'ASSET list stays local.',
    'LAUNCH readiness stays local.',
  ].join('\n');
}

describe('heavy full audit', { concurrency: 1 }, () => {
  test('a heavy audit stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-haudit-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-haudit-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-haudit-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-haudit-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-haudit-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-haudit-west-'));
    ...
    assert.equal(unsetOut.error, 'heavy_audit_root_required');
    ...
    assert.equal(homeOut.error, 'heavy_audit_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-FULL-AUDIT.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-FULL-AUDIT-REPLY.md'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'heavy_audit_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-FULL-AUDIT-REPLY.md')), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-FULL-AUDIT-BRIEF.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'AUDIT notes only.');
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-FULL-AUDIT-REPLY.md')), false);
    assert.equal(fs.existsSync(path.join(bad, 'DEMIGOD-FULL-AUDIT.json')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('META'), true);
    assert.equal(badStored.missing.includes('ASSET'), true);
    assert.equal(badStored.missing.includes('LAUNCH'), true);
    ...
    assert.equal(stored.markers.META, true);
    assert.equal(stored.markers.ASSET, true);
    assert.equal(stored.markers.LAUNCH, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'heavy-full-audit', 'audit.shot'), 'utf8');
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-FULL-AUDIT-REPLY.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'heavy-full-audit', 'audit.shot')), false);
