...
const CLI = path.join(OPS, 'demigod-heavy-copy-inventory.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/heavy-copy-audit';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'heavy-copy-audit');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-COPY-AUDIT.json',
  '/home/potter/HEAVY-COPY-AUDIT-REPLY.md',
  path.join(OPS, 'DEMIGOD-HEAVY-COPY-AUDIT.json'),
  path.join(OPS, 'HEAVY-COPY-AUDIT-REPLY.md'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-copy-inventory') || name.startsWith('HEAVY-COPY-AUDIT')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-COPY-AUDIT.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-COPY-INVENTORY.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'COPY notes stay local.',
    'KEEP the proof line.',
    'REWRITE the stale line.',
    'DELETE the extra chrome.',
    'CANVAS stays untouched.',
  ].join('\n');
}

describe('heavy copy audit', { concurrency: 1 }, () => {
  test('a copy audit stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-copy-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-copy-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-copy-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-copy-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-copy-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-copy-west-'));
    ...
    const outsideFile = path.join(outside, 'inventory.md');
    fs.writeFileSync(outsideFile, 'Outside keep secret\nCOPY KEEP REWRITE DELETE CANVAS\n');
    ...
    assert.equal(unsetOut.error, 'copy_audit_root_required');
    ...
    assert.equal(homeOut.error, 'copy_audit_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-COPY-AUDIT.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-COPY-AUDIT-REPLY.md'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'copy_audit_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-COPY-AUDIT-REPLY.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-COPY-INVENTORY.md'));
    const linked = run(leak, [], {}, cwdDir);
    const linkedOut = readJson(linked, 'symlink inventory');
    ...
    plant(bad, 'Harbor Bad keep', 'COPY notes only.');
    const badRun = run(bad, [], {}, cwdDir);
    assert.equal(badRun.status, 1, `thin inventory should fail\n${badRun.stdout}\n${badRun.stderr}`);
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-COPY-AUDIT-REPLY.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('REWRITE'), true);
    assert.equal(badStored.missing.includes('DELETE'), true);
    assert.equal(badStored.missing.includes('CANVAS'), true);
    ...
    assert.equal(stored.markers.COPY, true);
    assert.equal(stored.markers.KEEP, true);
    assert.equal(stored.markers.REWRITE, true);
    assert.equal(stored.markers.DELETE, true);
    assert.equal(stored.markers.CANVAS, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'heavy-copy-audit', 'audit.shot'), 'utf8');
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-COPY-AUDIT-REPLY.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'heavy-copy-audit', 'audit.shot')), false);
