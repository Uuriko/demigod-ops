...
export const AWARD_ROOM_CSS = 'header.dasha-hero,#dasha-tape,#simp,#chess,#faucet,#token{position:relative}header.dasha-hero::before,#dasha-tape::before,#simp::before,#chess::before,#faucet::before,#token::before{position:absolute;left:1rem;top:1.1rem;color:#dfff00;font:700 11px/1 Arial,Helvetica,sans-serif;letter-spacing:.12em}header.dasha-hero::before{content:"[01]"}#dasha-tape::before{content:"[02]"}#simp::before{content:"[03]"}#chess::before{content:"[04]"}#faucet::before{content:"[05]"}#token::before{content:"[06]"}#simp{min-height:100svh;box-sizing:border-box;padding:4.5rem 1.25rem 8rem}#dasha-tape,#chess,#faucet,#token{box-sizing:border-box;padding:3.5rem 1.25rem 8rem}#simp,#chess,#faucet,#token{opacity:0;transform:translateY(16px);transition:opacity .4s ease,transform .4s ease}#simp.is-in,#chess.is-in,#faucet.is-in,#token.is-in{opacity:1;transform:none}@media(prefers-reduced-motion:reduce){#simp,#chess,#faucet,#token{opacity:1;transform:none;transition:none}}';
