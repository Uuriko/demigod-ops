...
export const AWARD_ROOM_CSS = 'header.dasha-hero,#simp,#faucet,#token{position:relative}header.dasha-hero::before,#simp::before,#faucet::before,#token::before{position:absolute;left:1rem;top:1.1rem;color:#dfff00;font:700 11px/1 Arial,Helvetica,sans-serif;letter-spacing:.12em}header.dasha-hero::before{content:"[01]"}#simp::before{content:"[02]"}#faucet::before{content:"[03]"}#token::before{content:"[04]"}#simp{min-height:100svh;box-sizing:border-box;padding:4.5rem 1.25rem 8rem}#faucet,#token{box-sizing:border-box;padding:3.5rem 1.25rem 8rem}#simp,#faucet,#token{opacity:0;transform:translateY(16px);transition:opacity .4s ease,transform .4s ease}#simp.is-in,#faucet.is-in,#token.is-in{opacity:1;transform:none}@media(prefers-reduced-motion:reduce){#simp,#faucet,#token{opacity:1;transform:none;transition:none}}';
