...
const CLI = path.join(OPS, 'demigod-heavy-creative-next.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/creative-next';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'creative-next');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-CREATIVE-NEXT.json',
  '/home/potter/HEAVY-CREATIVE-NEXT.md',
  '/home/potter/HEAVY-CREATIVE-NEXT-SENT.txt',
  '/home/potter/DEMIGOD-CREATIVE-IMPLEMENT-PROMPT.md',
  '/home/potter/DEMIGOD-LEVERAGE-STATUS.json',
  path.join(OPS, 'DEMIGOD-HEAVY-CREATIVE-NEXT.json'),
  path.join(OPS, 'HEAVY-CREATIVE-NEXT.md'),
  path.join(OPS, 'HEAVY-CREATIVE-NEXT-SENT.txt'),
  path.join(OPS, 'DEMIGOD-CREATIVE-IMPLEMENT-PROMPT.md'),
  path.join(OPS, 'DEMIGOD-LEVERAGE-STATUS.json'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-creative-next') || name.startsWith('HEAVY-CREATIVE-NEXT')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-CREATIVE-NEXT.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'DEMIGOD-LEVERAGE-STATUS.json'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'CREATIVE notes stay local.',
    'BOARD stays in the root.',
    'SEED roles stay counted.',
    'PLACEMENT count stays local.',
    'PROOF stays on disk.',
  ].join('\n');
}

describe('creative next', { concurrency: 1 }, () => {
  test('a creative next record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-next-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-next-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-next-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-next-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-next-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-next-west-'));
    ...
    const outsideFile = path.join(outside, 'status.json');
    fs.writeFileSync(outsideFile, 'Outside keep secret\nCREATIVE BOARD SEED PLACEMENT PROOF\n');
    ...
    assert.equal(src.includes('demigod-leverage-status'), false);
    ...
    assert.equal(unsetOut.error, 'creative_root_required');
    ...
    assert.equal(homeOut.error, 'creative_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-CREATIVE-NEXT.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-CREATIVE-NEXT.md'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-CREATIVE-NEXT-SENT.txt'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'creative_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-CREATIVE-NEXT.md')), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-CREATIVE-NEXT-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(east, 'DEMIGOD-CREATIVE-IMPLEMENT-PROMPT.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'DEMIGOD-LEVERAGE-STATUS.json'));
    const linked = run(leak, [], {}, cwdDir);
    const linkedOut = readJson(linked, 'symlink status');
    ...
    plant(bad, 'Harbor Bad keep', 'CREATIVE notes only.');
    const badRun = run(bad, [], {}, cwdDir);
    assert.equal(badRun.status, 1, `thin status should fail\n${badRun.stdout}\n${badRun.stderr}`);
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-CREATIVE-NEXT.md')), false);
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-CREATIVE-NEXT-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(bad, 'DEMIGOD-CREATIVE-IMPLEMENT-PROMPT.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('SEED'), true);
    assert.equal(badStored.missing.includes('PLACEMENT'), true);
    assert.equal(badStored.missing.includes('PROOF'), true);

    plant(east, 'Harbor East keep', goodBody());
    const eastRun = run(east, [], {}, cwdDir);
    assert.equal(eastRun.status, 0, `east record failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.CREATIVE, true);
    assert.equal(stored.markers.BOARD, true);
    assert.equal(stored.markers.SEED, true);
    assert.equal(stored.markers.PLACEMENT, true);
    assert.equal(stored.markers.PROOF, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'creative-next', 'next.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west record failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-CREATIVE-NEXT.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-CREATIVE-NEXT-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'creative-next', 'next.shot')), false);
