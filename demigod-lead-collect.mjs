 ...
 *
 * These exist because the handle is derived from the x.com/<author>/status URL, so an account
 * that merely REPORTS someone else's hiring becomes the recipient. That shipped: draft
 * `x-tensorlake` pitched Demigod to @theconsensusdev, an infrastructure-newsletter account, about
 * a role at TensorLake — who could not hire anyone. Same shape as the recruiting-agency recipient
 * in demigod-demand.mjs's `recipient_is_agency`.
 *
 * ponytail: hand-maintained denylist, so it is whack-a-mole by construction. The real fix is to
 * read the employer out of the post text — an "@X is hiring" mention that differs from the author
 * IS the employer — but attachPublicContact only receives url/source, not post body. Plumb the
 * text through here before generalising. Do NOT reach for handle-vs-company name similarity: it
 * false-positives on every founder whose handle is their own name.
 */
export const NOISE_HANDLE_RE =
  /^(ycombinator|sfsoftwarejobs|securityblvd|linkedin|indeed|wellfound|angellist|builtin|techcrunch|producthunt|github|stackoverflow|workatastartup|theconsensusdev|jobs?with\w*|hiring\w*)$/i;
