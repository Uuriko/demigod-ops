...
export const TOOLS = [
  ...
  { id: 'identity-review', name: 'Identity review candidates', group: 'session', cmd: 'node demigod-identity-review.mjs', purpose: 'Companies that may be one company listed twice. Cross-source dedupe keys only on website host, so a row with NO website can never merge — that blind spot is the candidate class. Reports evidence and merges NOTHING: distinct companies share names (Atlas/Alex/Candor are real counterexamples in the live map), so a name-based auto-merge would destroy them', out: 'stdout (--json)', hot: false, safe: true },
