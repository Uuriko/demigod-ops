...
export const TOOLS = [
  ...
  { id: 'full-check', name: 'Full check', group: 'session', cmd: 'bin/dg full-check', purpose: 'Compatibility alias for bin/dg check full', out: '/tmp/dg-busy/full-check.json', alias: 'check' },
  ...
  { id: 'usertest-quick', name: 'User-test quick', group: 'session', cmd: 'bin/dg-usertest --quick', purpose: 'Faster UX suite without full selftest', out: '/tmp/dg-busy/user-test-latest.json' },
  ...
  { id: 'live-doctor', name: 'Live doctor (alias)', group: 'session', cmd: 'bin/dg live', purpose: 'Alias of truth', out: '/tmp/dg-busy/truth.json', alias: 'truth' },
  ...
  { id: 'orca-status', name: 'Orca status', group: 'orca', cmd: 'bin/dg-orca status', purpose: 'Runtime + Claude/Codex channel + keep-awake', out: '/tmp/dg-busy/orca-status.json' },
  ...
  { id: 'review-since', name: 'Review since HEAD~1', group: 'session', cmd: 'bin/dg-review --since HEAD~1 --no-contract --fail-on high', purpose: 'Agent thrash↓ delta review', out: '/tmp/dg-busy/review-latest.json' },
  ...
  { id: 'dg-publish', name: 'Publish front door', group: 'ship', cmd: 'bin/dg-publish --dry-run', purpose: 'Fail-closed live publish (auth+CDP+gates); dry-run default for agents', out: '/tmp/dg-busy/dg-publish.log' },
  ...
  { id: 'cycle-status', name: 'Cycle status', group: 'session', cmd: 'bin/dg cycle-status', purpose: 'Read-only cycle + demand hygiene + canonical NEXT status; --attest fails closed', out: '/tmp/dg-busy/cycle-status.json' },
  { id: 'cycle-work', name: 'Cycle work (one unit)', group: 'session', cmd: 'node demigod-cycle-work.mjs --domain=auto --owner=dashboard', purpose: 'One single-flight work unit (respects lock)', out: '/tmp/dg-busy/cycle-work-latest.json' },
  { id: 'never-stop-status', name: 'Never-stop status', group: 'session', cmd: 'node demigod-never-stop-loop.mjs status', purpose: 'Background backlog loop status' },
  { id: 'never-stop-stop', name: 'Stop never-stop', group: 'session', cmd: 'node demigod-never-stop-loop.mjs stop', purpose: 'Stop the background backlog loop' },
  { id: 'swarm-status', name: 'Swarm status', group: 'swarm', cmd: 'node demigod-swarm-busy.mjs status', purpose: 'Agent swarm supervisor status (paused by default)' },
  { id: 'swarm-stop', name: 'Stop swarm', group: 'swarm', cmd: 'node demigod-swarm-busy.mjs stop', purpose: 'Stop the agent swarm supervisor' },
  { id: 'codex-swarm', name: 'Codex swarm assist', group: 'swarm', cmd: 'bin/dg-codex-swarm status', purpose: 'Periodic Codex assist wave status; once/start/stop via bin/dg-codex-swarm (usage-limited until provider credits)', out: '/tmp/dg-busy/swarm/swarm-last.json' },
  { id: 'harness-selftest', name: 'Harness selftest', group: 'gates', cmd: 'node demigod-harness-selftest.mjs', purpose: 'Workloop harness contract selftest' },
  ...
  { id: 'dogfood', name: 'Tool dogfood', group: 'session', cmd: 'bin/dg dogfood', purpose: 'Real executions, judgments, synthetic probes, and unused-tool telemetry', out: '/tmp/dg-busy/tool-dogfood-status.json', hot: true },
  ...
  { id: 'role-ledger', name: 'Role first-seen ledger', group: 'session', cmd: 'node demigod-role-ledger.mjs report --posted', purpose: 'Per-role open-lifetime over 399 ATS boards; observedOpenDays uses OUR firstSeen (never board date); close only on successful-fetch-absence. report --posted = Greenhouse posting-age (attributed, evergreen-flagged)', out: 'DEMIGOD-ROLE-LEDGER.json', hot: false },
  ...
  { id: 'funnel-report', name: 'WIZ funnel report', group: 'session', cmd: 'node demigod-funnel-report.mjs', purpose: 'Anonymized WIZ funnel drop-off (start→step→complete, by device) from the live form-analytics store', out: '/tmp/dg-busy/form-analytics.json', hot: false },
  { id: 'honesty-audit', name: 'Live honesty audit', group: 'gates', cmd: 'node demigod-live-honesty-audit.mjs', purpose: 'Fetch live homepage as Googlebot; flag dishonest copy in crawler-visible content (scripts stripped). RED until source copy fixed', hot: false },
  ...
  { id: 'full-pass-status', name: 'Full-pass loop status', group: 'session', cmd: 'node demigod-full-pass-loop.mjs status', purpose: 'Durable multi-track loop (dash/webflow/frontend)', out: '/tmp/dg-busy/full-pass-state.json' },
  ...
  { id: 'truth-delta', name: 'Truth delta', group: 'gates', cmd: 'bin/dg ledger delta', purpose: 'What changed since last truth ledger line', out: '/tmp/dg-busy/version-ledger-tail.json' },
  ...
  { id: 'grok-out-contract', name: 'Grok-out projection contract', group: 'gates', cmd: 'node demigod-agent-dashboard.mjs --selftest-grok-out', purpose: 'Mailbox receipt contract (bold **VERDICT:** etc.); transport=ok ≠ incomplete' },
  { id: 'next-assert', name: 'NEXT identity assert', group: 'session', cmd: 'bin/dg next-canon --assert-same', purpose: 'Fail if control/cockpit/ship NEXT drift from buildNext', out: '/tmp/dg-busy/next.json' },
  ...
  { id: 'lock-who', name: 'Lock who', group: 'session', cmd: 'bin/dg lock-who', purpose: 'Who holds foot-core lock (pid/age/why)', out: '/tmp/dg-busy/foot-lock.json' },
  ...
  { id: 'match-review', name: 'Match review queue', group: 'session', cmd: 'bin/dg matches', purpose: 'Pair ledger review queue (not public board)', out: '/tmp/dg-busy/match-review-latest.json', hot: true },
  ...
  { id: 'audit-100', name: 'Audit 100 layers', group: 'gates', cmd: 'bin/dg audit-100', purpose: 'Multi-layer instrumented audit (honest residual list)', out: '/tmp/dg-busy/audit-100-latest.json' },
  ...
  { id: 'tools-registry', name: 'Tools registry', group: 'swarm', cmd: 'node demigod-tools-registry.mjs --md', purpose: 'This catalog' },
  { id: 'dash', name: 'Dashboard', group: 'swarm', cmd: 'bin/dg-dash', purpose: 'Agent dashboard UI :9878' },
  { id: 'api-coord', name: 'Coord API', group: 'swarm', cmd: 'curl -sS --max-time 5 http://127.0.0.1:9878/api/coord', purpose: 'Read-only coordination lanes, claims, and work-log health' },
  { id: 'agent-coord-status', name: 'Agent coordination status', group: 'swarm', cmd: 'bin/dg-agent-coord status', purpose: 'Read-only supervisor, worker, claim, and lane state' },
  { id: 'workflow-map', name: 'Workflow map', group: 'swarm', cmd: 'bin/dg-workflow-map status', purpose: 'Multi-agent workflow diagram status/update/review (occasional timer)', out: '/tmp/dg-busy/coord/workflow-map-last.json' },
