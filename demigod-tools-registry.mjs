...
export const TOOLS = [
  ...
  { id: 'structured-hiring', name: 'Role workspace / structured hiring', group: 'session', cmd: 'node demigod-structured-hiring.mjs status', purpose: 'Accepted-role + calibration + company context + candidate channels + review/relationship state; workspace --role=ID', out: '/tmp/dg-busy/structured-hiring-status.json', hot: true, safe: true },
  ...
  { id: 'action-authority', name: 'Action authority (DIE Action layer)', group: 'session', cmd: 'node demigod-action-authority.mjs --list', purpose: 'The 17 actions Demigod can take and the authority each needs. Fails closed on unknown; never-tier cannot be unlocked. check <action> for a verdict', out: null, safe: true },
  { id: 'posting-age-index', name: 'Posting-age index (publishable)', group: 'session', cmd: 'node demigod-posting-age-index.mjs --json', purpose: 'Share of attributable SF roles posted 90-365d, evergreen counted apart; limits ship with the number. Posting age, not a ghost-job claim', out: null, safe: true },
  ...
  { id: 'control', name: 'Control plane', group: 'session', cmd: 'bin/dg home', purpose: 'Cohesive map: site/webflow/match/review/hygiene/ship/agents', out: '/tmp/dg-busy/control-plane.json' },
  ...
  { id: 'agent-bus', name: 'Agent peer bus', group: 'session', cmd: 'bin/dg-bus status', purpose: 'Local Claude/Codex/Grok tasks with atomic filesystem receipts', out: '/tmp/dg-busy/agent-bus', hot: true },
  ...
  { id: 'enrichment-clay', name: 'Clay website enrich', group: 'session', cmd: 'node demigod-enrichment.mjs clay', purpose: 'Website-facing Clay: roles feed + public-roles embed + velocity + gated requisitions (public ATS facts only)', out: '/tmp/dg-busy/enrichment-clay-website.json', safe: true },
  { id: 'enrichment-feed', name: 'Public roles feed', group: 'session', cmd: 'node demigod-enrichment.mjs feed --days 1 --limit 120', purpose: 'DEMIGOD-ROLES-FEED.json + public-roles embed for observed-roles strip', out: 'DEMIGOD-ROLES-FEED.json', safe: true },
  { id: 'enrichment-coverage', name: 'Enrichment coverage/freshness', group: 'session', cmd: 'node demigod-enrichment.mjs coverage', purpose: 'Public-field fill rates + dual-clock maintained-stale (no quality scores)', out: '/tmp/dg-busy/enrichment-coverage.json', safe: true },
  { id: 'enrichment-velocity', name: 'Hiring velocity (ledger)', group: 'session', cmd: 'node demigod-enrichment.mjs velocity --days 7', purpose: 'Open/close counts from firstSeen/closedAt (observation clock)', out: '/tmp/dg-busy/enrichment-velocity.json', safe: true },
  ...
  { id: 'doctor', name: 'Doctor', group: 'session', cmd: 'node demigod-doctor.mjs', purpose: 'Environment health: CDP, dashboard, keys and required binaries', out: '/tmp/dg-busy/doctor.json' },
  ...
  { id: 'startup-map-rebuild', name: 'SF startup map rebuild + jobs', group: 'session', cmd: 'node demigod-startup-map-data.mjs --with-jobs', purpose: 'Map rebuild including jobs attach pass (heavier; public open data only)', out: 'DEMIGOD-SF-STARTUP-MAP.json', hot: false },
  { id: 'conversion-playtest', name: 'Conversion playtest', group: 'gates', cmd: 'node demigod-conversion-playtest.mjs --pw --local', purpose: 'Playwright conversion-path playtest (local foot inject; no submit)', out: '/tmp/dg-busy/conversion-playtest.json', hot: false, safe: true },
  { id: 'startups-static-paste', name: 'Startups static paste check', group: 'site', cmd: 'node demigod-startups-static-paste.mjs --check', purpose: 'Read-only: Designer /startups custom-code vs sealed static (no save). --save is Designer mutate + publish-auth gated.', out: null, hot: false, safe: true },
  { id: 'startups-static-paste-save', name: 'Startups static paste save', group: 'ship', cmd: 'node demigod-startups-static-paste.mjs --save', purpose: 'Write sealed directory HTML into Webflow /startups page custom code (Designer mutate; needs publish auth + foot lock)', mutate: true, hot: false },
  ...
  { id: 'roles-pipeline', name: 'Multi-source roles pipeline', group: 'session', cmd: 'node demigod-roles-pipeline.mjs', purpose: 'X+HN hiring discover → ATS board attach → role-ledger poll → roles-feed → public-roles embed → directory static. Honest: only public ATS roles reach the site; X posts only surface boards. Live CDN still needs ship.', out: 'DEMIGOD-PUBLIC-ROLES.json', hot: true },
  ...
  { id: 'public-roles', name: 'Public roles homepage embed', group: 'session', cmd: 'node demigod-public-roles.mjs', purpose: 'Build DEMIGOD-PUBLIC-ROLES.json + footer embed for observed-roles rail (SF/startup ranked; not matching inventory)', out: 'DEMIGOD-PUBLIC-ROLES.json', safe: true },
  ...
  // High-frequency quality wraps — keep ids aligned with dogfood --tool= labels so status is not “unregistered”
  { id: 'navigation-audit', name: 'Navigation audit', group: 'gates', cmd: 'node demigod-navigation-audit.mjs', purpose: 'Rendered sitemap/control audit (read-only CDP; no form submit)', out: '/tmp/dg-busy/navigation-audit.json', safe: true },
  { id: 'design-audit', name: 'Design audit', group: 'gates', cmd: 'node demigod-design-audit.mjs --quick', purpose: 'Screenshots + off-palette scan on key routes', out: 'DEMIGOD-DESIGN-AUDIT.json', safe: true },
  { id: 'lighthouse', name: 'Lighthouse mobile smoke', group: 'gates', cmd: 'node demigod-lighthouse.mjs --path=/', purpose: 'Mobile Lighthouse on a live route (performance budget gate when scored)', out: '/tmp/dg-busy/lighthouse-latest.json', safe: true },
  { id: 'mobile-a11y', name: 'Mobile a11y sweep', group: 'gates', cmd: 'node demigod-cdp-mobile-a11y-sweep.mjs', purpose: 'CDP mobile accessibility sweep across public routes', out: '/tmp/dg-busy/mobile-a11y-sweep.json', safe: true },
  { id: 'seo-audit', name: 'SEO structure audit', group: 'gates', cmd: 'node demigod-seo-audit.mjs', purpose: 'Live route SEO/structure audit (titles, headings, meta honesty)', out: '/tmp/dg-busy/seo-audit.json', safe: true },
  { id: 'form-e2e', name: 'Form e2e path', group: 'forms', cmd: 'node demigod-form-e2e.mjs', purpose: 'CDP WIZ open/drive path proof (default: no live submit)', out: '/tmp/dg-busy/form-e2e-latest.json', safe: true },
  { id: 'forms-full-audit', name: 'Forms full audit', group: 'forms', cmd: 'node demigod-forms-full-audit.mjs --local', purpose: 'Full rendered forms/controls audit (local foot inject; no submit)', out: '/tmp/dg-busy/forms-full-audit.json', hot: false, safe: true },
  { id: 'wizard-playtest', name: 'Wizard playtest', group: 'forms', cmd: 'node demigod-wizard-playtest.mjs --desktop-only', purpose: 'Typeform-style WIZ step/review/nav CDP playtest', out: 'DEMIGOD-WIZARD-PLAYTEST.json', safe: true },
  { id: 'axe-routes', name: 'Axe routes a11y', group: 'gates', cmd: 'node demigod-axe-routes.mjs', purpose: 'axe-core a11y smoke on live/local routes via Playwright', out: '/tmp/dg-busy/axe-routes.json', safe: true },
  { id: 'button-audit', name: 'Button/link audit', group: 'gates', cmd: 'node demigod-button-audit.mjs', purpose: 'Full-site interactive control audit on key routes (CDP, read-only)', out: '/tmp/dg-busy/button-audit.json', safe: true },
  { id: 'playtest-review', name: 'Playtest review', group: 'gates', cmd: 'node demigod-playtest-review.mjs', purpose: 'Aggregate recent playtest/audit receipts into an operator review card', out: '/tmp/dg-busy/playtest-review.json', hot: false, safe: true },
  { id: 'truth-final', name: 'Truth require-match', group: 'gates', cmd: 'bin/dg truth --require-match', purpose: 'Truth oracle with disk==live version required (release-mode check)', out: '/tmp/dg-busy/truth.json', safe: true },
  { id: 'cm6-publish-only', name: 'CM6 publish-only', group: 'ship', cmd: 'node demigod-cm6-paste-publish.mjs --publish-only', purpose: 'Webflow publish step only after paste is already on disk/designer; mutates live', mutate: true, hot: false },
  ...
  { id: 'blog-sync', name: 'Blog sync check', group: 'site', cmd: 'node demigod-blog-sync.mjs --check', purpose: 'Fan-out SoR demigod-blog-posts.json → foot/head stay in sync (check only by default)', out: null, safe: true },
  ...
  { id: 'grok-ask', name: 'Ask Grok (stateless)', group: 'session', cmd: 'bin/grok-ask', purpose: 'Stateless advisory consult; never grants publish/outbound/money authority', hot: false, safe: true },
  ...
  { id: 'company-intelligence', name: 'Company intelligence', group: 'session', cmd: 'node demigod-company-intelligence.mjs list --limit=10', purpose: 'Private exact-identity company list; get, enrich, memo, and writeback stay read-only/dry-run', safe: true },
