...
export const TOOLS = [
  ...
  { id: 'enrichment-scoreboard', name: 'Enrichment scoreboard', group: 'session', cmd: 'node demigod-enrichment.mjs scoreboard', purpose: 'Ledger/map/export enrichment counts + 7d boardActivity (newOpen/boardExit; exits≠filled; history caveats)', out: '/tmp/dg-busy/enrichment-scoreboard.json', safe: true },
  { id: 'ledger-reclassify-dry', name: 'Ledger fn reclassify (dry-run)', group: 'session', cmd: 'node demigod-enrichment.mjs reclassify --dry-run', purpose: 'Offline measure ledger.fn vs live categorizeRole without writing (AR-08 drift)', out: '/tmp/dg-busy/ledger-fn-reclassify.json', safe: true },
  { id: 'ledger-reclassify', name: 'Ledger fn reclassify (write)', group: 'session', cmd: 'node demigod-enrichment.mjs reclassify', purpose: 'Offline write ledger.fn from titles (no network); follow with map enrich-map when roleMix stale', out: '/tmp/dg-busy/ledger-fn-reclassify.json', mutate: true, safe: false },
  { id: 'map-role-mix-enrich', name: 'Map roleMix enrich (offline)', group: 'session', cmd: 'node demigod-directory-aging.mjs --enrich-map', purpose: 'Stamp map aging+roleMix from reclassed ledger (board-join); enqueues research reseal', out: 'DEMIGOD-SF-STARTUP-MAP.json', mutate: true, safe: false },
  ...
  { id: 'roles-feed', name: 'Public roles feed + hiring brief', group: 'session', cmd: 'node demigod-roles-feed.mjs --days 1', purpose: 'Feed v8 plus read-only exact facets and --brief text output (--company/--fn/--provider/--seniority/--us-only; query modes never overwrite the canonical feed). firstObservedAt vs postedAt stay distinct; no ranking, intent, or filled/hired claim', out: 'DEMIGOD-ROLES-FEED.json', hot: false, safe: true },
  ...
  { id: 'hiring-pulse', name: 'SF Hiring Pulse (data-media)', group: 'session', cmd: 'node demigod-hiring-pulse.mjs', purpose: 'Shareable Pulse: batch-curve, AI+PeopleOps share, board activity (new open + board-exit with short-history caveats; exits ≠ filled), roleMix, top hirers, honest CTA', out: 'hiring-pulse.html', hot: false },
  { id: 'directory-static', name: 'Crawlable static directory', group: 'session', cmd: 'node demigod-directory-static.mjs', purpose: 'Webflow footer fragment with every verified hirer + capped ItemList JSON-LD (safe links, under 50KB)', out: 'sf-startups-static.html', hot: false },
