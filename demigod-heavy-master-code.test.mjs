...
const CLI = path.join(OPS, 'demigod-heavy-master-code.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/master-code';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'master-code');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-MASTER-CODE.json',
  '/home/potter/HEAVY-MASTER-CODE-HELP.md',
  path.join(OPS, 'DEMIGOD-HEAVY-MASTER-CODE.json'),
  path.join(OPS, 'HEAVY-MASTER-CODE-HELP.md'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-master-code') || name.startsWith('DEMIGOD-HEAVY-MASTER-CODE') || name.startsWith('HEAVY-MASTER-CODE-HELP')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-MASTER-CODE.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-MASTER-CODE-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'MASTER notes stay local.',
    'NAV stays on disk.',
...
describe('master code', { concurrency: 1 }, () => {
  test('a master record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-mst-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-mst-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-mst-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-mst-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-mst-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-mst-west-'));
    ...
    assert.equal(unsetOut.error, 'master_root_required');
    ...
    assert.equal(homeOut.error, 'master_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-MASTER-CODE.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-MASTER-CODE-HELP.md'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'master_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-MASTER-CODE-HELP.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-MASTER-CODE-HELP.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-MASTER-CODE-SOURCE.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'MASTER notes only.');
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-MASTER-CODE-HELP.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('NAV'), true);
    ...
    assert.equal(eastRun.status, 0, `east master failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.NAV, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'master-code', 'master.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west master failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-MASTER-CODE-HELP.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'master-code', 'master.shot')), false);
