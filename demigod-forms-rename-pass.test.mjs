...
const CLI = path.join(OPS, 'demigod-forms-rename-pass.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/forms-rename-pass';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'forms-rename-pass');
const SHARED = [
  '/home/potter/DEMIGOD-FORMS-RENAME.json',
  path.join(OPS, 'DEMIGOD-FORMS-RENAME.json'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-forms-rename') || name.startsWith('DEMIGOD-FORMS-RENAME')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-FORMS-RENAME.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-FORMS-RENAME-PASS-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'FORMS notes stay local.',
...
describe('forms rename pass', { concurrency: 1 }, () => {
  test('a forms rename record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-frp-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-frp-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-frp-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-frp-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-frp-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-frp-west-'));
    ...
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-FORMS-RENAME.json'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-FORMS-RENAME-PASS-SOURCE.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'FORMS notes only.');
    ...
    assert.equal(eastRun.status, 0, `east forms failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.FORMS, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'forms-rename-pass', 'rename.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west forms failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'forms-rename-pass', 'rename.shot')), false);
