...
const CLI = path.join(OPS, 'demigod-heavy-grok-perf.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/grok-perf';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'grok-perf');
const SHARED = [
  '/home/potter/DEMIGOD-GROK-PERF.json',
  '/home/potter/HEAVY-GROK-BUILD-PERF.md',
  path.join(OPS, 'DEMIGOD-GROK-PERF.json'),
  path.join(OPS, 'HEAVY-GROK-BUILD-PERF.md'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-grok-perf') || name.startsWith('HEAVY-GROK-BUILD-PERF')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-GROK-PERF.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-GROK-PERF-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'PERF notes stay local.',
    'CONFIG stays on disk.',
    'MCP trim stays planted.',
    'TAB hygiene stays local.',
    'METRIC stays local.',
  ].join('\n');
}

describe('grok perf', { concurrency: 1 }, () => {
  test('a perf record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-gperf-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-gperf-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-gperf-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-gperf-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-gperf-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-gperf-west-'));
    ...
    assert.equal(unsetOut.error, 'perf_root_required');
    ...
    assert.equal(homeOut.error, 'perf_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-GROK-PERF.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-GROK-BUILD-PERF.md'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'perf_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-GROK-BUILD-PERF.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-GROK-PERF-SOURCE.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'PERF notes only.');
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-GROK-BUILD-PERF.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('CONFIG'), true);
    assert.equal(badStored.missing.includes('MCP'), true);
    assert.equal(badStored.missing.includes('TAB'), true);
    assert.equal(badStored.missing.includes('METRIC'), true);

    plant(east, 'Harbor East keep', goodBody());
    const eastRun = run(east, [], {}, cwdDir);
    assert.equal(eastRun.status, 0, `east perf failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.PERF, true);
    assert.equal(stored.markers.CONFIG, true);
    assert.equal(stored.markers.MCP, true);
    assert.equal(stored.markers.TAB, true);
    assert.equal(stored.markers.METRIC, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'grok-perf', 'perf.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west perf failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-GROK-BUILD-PERF.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'grok-perf', 'perf.shot')), false);
