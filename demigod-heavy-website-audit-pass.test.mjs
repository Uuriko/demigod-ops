...
const CLI = path.join(OPS, 'demigod-heavy-website-audit-pass.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/website-audit-pass';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'website-audit-pass');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-WEBSITE-AUDIT.json',
  '/home/potter/HEAVY-WEBSITE-AUDIT-2026.md',
  '/home/potter/HEAVY-FULL-AUDIT-BRIEF.md',
  path.join(OPS, 'DEMIGOD-HEAVY-WEBSITE-AUDIT.json'),
  path.join(OPS, 'HEAVY-WEBSITE-AUDIT-2026.md'),
  path.join(OPS, 'HEAVY-FULL-AUDIT-BRIEF.md'),
  path.join(OPS, 'HEAVY-DEMIGOD-CODE-HELP.md'),
  path.join(OPS, 'HEAVY-DEMIGOD-DESIGN-AUDIT.md'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-website-audit') || name.startsWith('DEMIGOD-HEAVY-WEBSITE-AUDIT') || name.startsWith('HEAVY-WEBSITE-AUDIT')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-WEBSITE-AUDIT.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-WEBSITE-AUDIT-PASS-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'WEBSITE notes stay local.',
    'AUDIT stays on disk.',
    'CODE stays planted.',
    'DESIGN stays local.',
...
describe('website audit pass', { concurrency: 1 }, () => {
  test('a website audit record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-wap-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-wap-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-wap-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-wap-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-wap-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-wap-west-'));
    ...
    assert.equal(src.includes('runHeavyCodeHelp'), false);
    assert.equal(src.includes('runHeavyDesignAudit'), false);
    assert.equal(src.includes('captureDemigodScreenshots'), false);
    assert.equal(src.includes('demigod-full-audit'), false);
    assert.equal(src.includes('demigod-copy-inventory'), false);
    ...
    assert.equal(unsetOut.error, 'website_root_required');
    ...
    assert.equal(homeOut.error, 'website_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-WEBSITE-AUDIT.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-WEBSITE-AUDIT-2026.md'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-FULL-AUDIT-BRIEF.md'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'website_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-WEBSITE-AUDIT-2026.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-FULL-AUDIT-BRIEF.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-DEMIGOD-CODE-HELP.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-DEMIGOD-DESIGN-AUDIT.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-WEBSITE-AUDIT-2026.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    const verify = run(east, ['--verify'], {}, cwdDir);
    const verifyOut = readJson(verify, 'verify');
    assert.equal(verify.status, 1);
    assert.equal(verifyOut.error, 'publish_refused');
    assert.equal(verifyOut.liveFetch, false);
    assert.equal(fs.existsSync(reportFile(east)), false);

    const capture = run(east, ['--capture'], {}, cwdDir);
    const captureOut = readJson(capture, 'capture');
    assert.equal(capture.status, 1);
    assert.equal(captureOut.error, 'publish_refused');
    assert.equal(captureOut.aiSubmit, false);
    assert.equal(fs.existsSync(reportFile(east)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-WEBSITE-AUDIT-PASS-SOURCE.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'WEBSITE notes only.');
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-WEBSITE-AUDIT-2026.md')), false);
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-FULL-AUDIT-BRIEF.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-WEBSITE-AUDIT-2026.md')), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-FULL-AUDIT-BRIEF.md')), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-DEMIGOD-CODE-HELP.md')), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-DEMIGOD-DESIGN-AUDIT.md')), false);
    ...
    assert.equal(stored.markers.WEBSITE, true);
    assert.equal(stored.markers.AUDIT, true);
    assert.equal(stored.markers.CODE, true);
    assert.equal(stored.markers.DESIGN, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'website-audit-pass', 'pass.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west website failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-WEBSITE-AUDIT-2026.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-FULL-AUDIT-BRIEF.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'website-audit-pass', 'pass.shot')), false);
