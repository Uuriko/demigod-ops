...
<script>(()=>{const href="/studio#look=ticket";document.querySelectorAll('a[href^="/studio"],a[href*="/studio#"]');})();</script>
  ...
  const assertHomeCalmCss = (html, label) => {
    ...
    assert.match(css, /#stills::before\{content:"\[03\]"/, `${label} must number the stills strip`);
    assert.match(css, /#simp::before\{content:"\[04\]"/, `${label} must number the board room`);
    assert.match(css, /#chess::before\{content:"\[05\]"/, `${label} must number the chess room`);
    assert.match(css, /#faucet::before\{content:"\[06\]"/, `${label} must number the faucet room`);
    assert.match(css, /#token::before\{content:"\[07\]"/, `${label} must number the mint band`);
  ...
  const assertHomeFirst = (html, label) => {
    ...
    assert.match(html, /class="stills-grid"/, `${label} stills are a quiet first-party grid`);
    ...
    assert.doesNotMatch(hero, /id="dasha-simp-board"|simp-board\.js|class="dasha-quiz"|dasha-faucet|faucet\.js|dasha-tape|dexscreener|id="chess"|id="board"|gate-action|id="stills"|stills-grid/, `${label} hero first paint is not the board, faucet, chart, chess, or stills`);
