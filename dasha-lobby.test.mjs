...
assert.doesNotMatch(chessPage, /\/airdrop|\/earn|\/claim|\/graph/, 'chess chrome must not grow dead or shelved doors');
{
  ...
  assert.equal(DASHA_ROOMS.length, 10, 'live room list is the ten public rooms');
...
assert.match(page, /<b>\[10\]<\/b> Privacy/, 'lobby rail ends at Privacy');
...
<footer><p><a href="/how-to-buy">How to buy</a> · <a href="https://lobby.getdasha.com/forum">Forum</a> · <a href="/lobby">Lobby</a> · <a href="/dasha">Desk</a> · <a href="https://lobby.getdasha.com/chess">Chess</a></p></footer>
  ...
  assert.doesNotMatch(injected.match(/<header class="dasha-slim">[\s\S]*?<\/header>/)?.[0] || '', /href="\/faucet"/, 'home hamburger must not door to Faucet');
  assert.doesNotMatch(injected.match(/<footer class="dasha-foot">[\s\S]*?<\/footer>/)?.[0] || '', /href="\/faucet"/, 'home footer must not door to Faucet');
...
${liveHomeFooter}
  ...
  for (const host of ['www.getdasha.com', 'lobby.getdasha.com']) {
    ...
    assert.match(howtoHtml, /href="\/forum">Forum</);
    assert.doesNotMatch(howtoHtml, /USDC/);
  ...
  assert.equal(ensurePrivacyLink(lobbyLinked), lobbyLinked, 'lobby nav Privacy inject must be idempotent');
  {
    ...
    assert.ok([...forumHtml.matchAll(/href=["']\/privacy["']/g)].length >= 1, 'lobby /forum must keep Privacy in chrome');
    ...
    assert.match(forumHtml, /<b>\[01\]<\/b> Studio/);
    assert.match(forumHtml, /<b>\[02\]<\/b> Forum/);
    assert.match(forumHtml, /class="dasha-next"[\s\S]*href="\/simp">Simp</);
    ...
    for (const host of ['www.getdasha.com', 'getdasha.com']) {
      for (const method of ['GET', 'HEAD']) {
        for (const path of ['/bounties', '/bounties/']) {
          ...
          assert.match(html, /href="\/forum">Forum</, `${host} ${path} menu must include Forum`);
          ...
          assert.doesNotMatch(html, /\/airdrop|\/earn|\/claim|\/graph/i, `${host} ${path} chrome must not grow dead or shelved doors`);
          ...
          assert.match(html, /href="\/forum">Forum</);
