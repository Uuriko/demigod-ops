...
assert.match(chessPage, /class="dasha-ham"/);
...
assert.match(page, /class="dasha-ham"/, 'lobby must use the hamburger, not a door strip');
...
assert.match(page, /class="dasha-next"[\s\S]*href="\/graph">Next up · Graph</, 'lobby next-up points at a live room');
...
for (const path of ['/no-such-page', '/no-such-page-242', '/no-such-page-251', '/no-such-page-253', '/no-such-page/']) {
  for (const method of ['GET', 'HEAD']) {
    ...
    if (method === 'HEAD') {
      ...
      assert.match(body, /class="dasha-ham"/);
...
}
{
  ...
  const assertSimpFirstHtml = (html, label) => {
    ...
    assert.match(html, /class="dasha-ham"/, `${label} must ship the hamburger`);
    ...
    assert.match(html, /href="\/chess">Chess</, `${label} menu sister is Chess`);
    assert.match(html, /class="buy-dasha dasha-ham-buy"[^>]*>Buy \$dasha</, `${label} must keep one acid Buy pill`);
...
<link rel="stylesheet" href="https://cdn.prod.website-files.com/css/webflow-https-errors.webflow.css">
  ...
  try {
    ...
    for (const host of ['www.getdasha.com', 'getdasha.com']) {
      ...
      assert.match(privacyHtml, /class="dasha-ham"/);
      assert.match(privacyHtml, /class="dasha-ticks"/);
    }
  } finally {
...
}
{
  ...
  const assertBountiesChrome = (section, label) => {
    ...
    assert.match(section, /class="dasha-ham"/, `${label} must ship the hamburger`);
    ...
    assert.match(section, /class="buy-dasha dasha-ham-buy"/, `${label} must keep one acid Buy pill`);
...

{
  ...
  assert.match(verseHtml, /class="dasha-ham"/);
  assert.match(verseHtml, /href="\/graph">Graph</);
  assert.match(verseHtml, /href="\/bounties">Bounties</);
  assert.match(verseHtml, /class="dasha-next"[\s\S]*href="\/graph">Next up · Graph</);
...
<footer><p><a href="/how-to-buy">How to buy</a> · <a href="https://lobby.getdasha.com/forum">Forum</a> · <a href="/lobby">Lobby</a> · <a href="/dasha">Desk</a> · <a href="https://lobby.getdasha.com/chess">Chess</a></p></footer>
  ...
  assert.match(injected, /class="dasha-ham"/, 'home rewrite must add the hamburger');
  assert.match(injected, /class="dasha-ticks"/, 'home rewrite must add crop marks');
  assert.doesNotMatch(injected.match(/<header class="dasha-ham">[\s\S]*?<\/header>/)?.[0] || '', /\/simp|\/bounties|\/privacy|\/airdrop/, 'home hamburger must stay four doors');
...
${liveHomeFooter}
  ...
  for (const host of ['www.getdasha.com', 'lobby.getdasha.com']) {
    ...
    assert.match(howtoHtml, /class="dasha-ham"/);
    assert.match(howtoHtml, /class="dasha-ticks"/);
    ...
    assert.match(chessHtml, /class="dasha-ham"/);
  ...
  for (const path of ['/lobby', '/lobby/']) {
    ...
    assert.match(lobbyHtml, /class="dasha-ham"/);
    ...
    assert.match(lobbyHtml, /class="dasha-next"[\s\S]*href="\/graph">Next up · Graph</);
    ...
    for (const host of ['www.getdasha.com', 'getdasha.com']) {
      for (const method of ['GET', 'HEAD']) {
        for (const path of ['/bounties', '/bounties/']) {
          ...
          assert.match(html, /class="dasha-ham"/, `${host} ${path} must ship the hamburger`);
