...
const CLI = path.join(OPS, 'demigod-board-write-guard.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/board-write-guard';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'board-write-guard');
const OPS_BOARD = path.join(OPS, 'DEMIGOD-BOARD.json');
const SHARED = [
  '/home/potter/DEMIGOD-BOARD-WRITE-GUARD.json',
  '/home/potter/DEMIGOD-BOARD.json',
  path.join(OPS, 'DEMIGOD-BOARD-WRITE-GUARD.json'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-board-write-guard') || name.startsWith('DEMIGOD-BOARD-WRITE-GUARD')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-BOARD-WRITE-GUARD.json');
}

function plant(dir, phrase, board) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'DEMIGOD-BOARD.json'), JSON.stringify({ ...board, note: phrase }));
}

function sampleBoard() {
  return {
    roles: [
      { sample: true, title: 'Founding designer' },
      { sample: true, title: 'Product manager' },
    ],
    receipts: [{ sample: true, id: 'sample-receipt' }],
    allowRealRoles: false,
    allowRealReceipts: false,
  };
}
...
function realBoard() {
  return {
    roles: [
      { sample: false, title: 'Real role' },
      { sample: true, title: 'Sample one' },
      { sample: true, title: 'Sample two' },
      { sample: true, title: 'Sample three' },
    ],
    receipts: [{ sample: false, id: 'real-receipt' }],
    allowRealRoles: false,
    allowRealReceipts: false,
  };
}

describe('board write guard', { concurrency: 1 }, () => {
  test('a guard record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-bwg-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-bwg-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-bwg-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-bwg-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-bwg-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-bwg-west-'));
    const sharedBefore = Object.fromEntries(SHARED.map((file) => [file, snapshot(file)]));
    const opsBoardBefore = snapshot(OPS_BOARD);
    ...
    fs.writeFileSync(outsideFile, JSON.stringify({
      note: 'Outside keep secret',
      roles: [{ sample: false, title: 'Outside role' }],
    }));
    ...
    assert.equal(src.includes('demigod-agent-tools-lib'), false);
    ...
    assert.equal(unsetOut.error, 'guard_root_required');
    ...
    assert.equal(homeOut.error, 'guard_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-BOARD-WRITE-GUARD.json'), false);
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-BOARD.json'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'guard_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    assert.equal(snapshot(OPS_BOARD), opsBoardBefore);
    ...
    assert.equal(liveOut.livePublish, false);
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal((live.stdout + live.stderr).includes('https://'), false);
    assert.equal((live.stdout + live.stderr).includes('grok.com'), false);

    const escaped = run(east, ['--json', outsideFile], {}, cwdDir);
    const escapedOut = readJson(escaped, 'outside json');
    assert.equal(escaped.status, 1);
    assert.equal(escapedOut.error, 'guard_root_required');
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal((escaped.stdout + escaped.stderr).includes('Outside keep secret'), false);
    ...
    plant(bad, 'Harbor Bad keep', realBoard());
    const badRun = run(bad, [], {}, cwdDir);
    assert.equal(badRun.status, 1, `real board should fail\n${badRun.stdout}\n${badRun.stderr}`);
    ...
    assert.equal(badOut.realRoles > 0, true);
    assert.equal(badOut.realReceipts > 0, true);
    assert.equal(badOut.roles > 3, true);
    ...
    assert.equal(badStored.issues.some((issue) => String(issue.msg).includes('realRoles')), true);
    assert.equal(badStored.issues.some((issue) => String(issue.msg).includes('realReceipts')), true);
    assert.equal(badStored.issues.some((issue) => String(issue.msg).includes('> 3 cap')), true);

    plant(east, 'Harbor East keep', sampleBoard());
    const eastRun = run(east, [], {}, cwdDir);
    assert.equal(eastRun.status, 0, `east guard failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.counts.roles, 2);
    assert.equal(stored.counts.realRoles, 0);
    assert.equal(JSON.stringify(stored).includes('Harbor West keep'), false);
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'board-write-guard', 'guard.shot'), 'utf8');
    ...
    plant(west, 'Harbor West keep', sampleBoard());
    const westRun = run(west, [], {}, OPS);
    assert.equal(westRun.status, 0, `west guard failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'board-write-guard', 'guard.shot')), false);
