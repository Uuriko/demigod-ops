...
Licenses + PII flagged (Demigod = no-PII + open/attributed data). All ATS feeds below are public
...
- Runners-up: Deno Deploy (1M req/mo, no CC), Val.town (tiny; public-code = on-brand).
...
- **GitHub Actions cron on a PUBLIC repo — top pick.** Unlimited free minutes; the daily job commits the
  updated first-seen ledger back (versioned + attributed — matches open-data ethos). Traps: private-repo
...
| **levels.fyi** | license forbids redistributing derived/aggregate data | ✗ AVOID public-facing — would violate their license + our open-data promise. |
...
Everything else is public-domain / CC0 / MIT / free-with-attribution — clean for the brand.
