#!/usr/bin/env node
/**
 * Local drift fix check in an explicit data root.
 * Writes DEMIGOD-DRIFT-FIX.json under DEMIGOD_ROOT. Does not open a browser or send a prompt.
 */
import fs from 'fs';
...
function reportPath() {
  return path.join(dataRoot(), 'DEMIGOD-DRIFT-FIX.json');
}
function shotDir() {
  return path.join(dataRoot(), 'audit-shots', 'drift-fix-pass');
}
...
function main() {
  ...
  if (!root || path.resolve(root) === '/home/potter' || path.resolve(root) === path.resolve(scriptDir())) {
    refuse('drift_root_required');
  }
  const footText = readText(root, 'demigod-foot-core.js');
  const notes = readText(root, 'HEAVY-DRIFT-FIX-PASS-SOURCE.md');
  ...
  const dir = shotDir();
  const shot = path.join(dir, 'drift.shot');
