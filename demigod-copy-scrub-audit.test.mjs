...
const CLI = path.join(OPS, 'demigod-copy-scrub-audit.mjs');
const SHARED = [
  '/home/potter/DEMIGOD-COPY-SCRUB-AUDIT.json',
  path.join(OPS, 'DEMIGOD-COPY-SCRUB-AUDIT.json'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-copy-scrub-audit-')).sort();
}

function plant(dir, phrase, html) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'demigod-copy-scrub-source.html'), html);
}
...
function goodHtml(phrase) {
  return [
    ...
    '<div id="dg-signal-bar">Live brief signal</div>',
    '<p>Submit brief or profile</p>',
  ].join('\n');
}

function badHtml() {
  return [
    '<p>startups receive 3-5</p>',
    '<p>lorem ipsum</p>',
  ].join('\n');
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-COPY-SCRUB-AUDIT.json');
}

describe('copy scrub', { concurrency: 1 }, () => {
  test('a copy scrub stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-scrub-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-scrub-bad-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-scrub-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-scrub-west-'));
    ...
    assert.equal(unsetOut.error, 'scrub_root_required');
    ...
    assert.equal(homeOut.error, 'scrub_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-COPY-SCRUB-AUDIT.json'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'scrub_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badRun.status, 1, `bad scrub should fail\n${badRun.stdout}\n${badRun.stderr}`);
    ...
    assert.equal(badOut.leaks, 3);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    const badStored = JSON.parse(fs.readFileSync(reportFile(bad), 'utf8'));
    assert.equal(badStored.leaks.some((leak) => leak.issue === 'volume_language'), true);
    assert.equal(badStored.leaks.some((leak) => leak.issue === 'lorem_placeholder'), true);
    assert.equal(badStored.leaks.some((leak) => leak.issue === 'fit_language_missing'), true);
    assert.equal(badStored.checks.hasVolumeLeak, true);
    assert.equal(badStored.checks.hasLorem, true);
    assert.equal(badStored.checks.hasGoodFitSteps, false);
    assert.equal(badStored.liveFetch, false);

    plant(east, 'Harbor East keep', goodHtml('Harbor East keep'));
    const eastRun = run(east, [], {}, cwdDir);
    assert.equal(eastRun.status, 0, `east scrub failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.checks.hasGoodFitSteps, true);
    assert.equal(stored.checks.hasInjectedSignalOrSteps, true);
    assert.equal(stored.checks.hasVolumeLeak, false);
    assert.equal(stored.checks.hasLorem, false);
    assert.equal(stored.leaks.length, 0);
    ...
    assert.equal(westRun.status, 0, `west scrub failed\n${westRun.stdout}\n${westRun.stderr}`);
