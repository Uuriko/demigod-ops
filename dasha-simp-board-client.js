 ...
 *
 * 2026-08-16: source reconstructed from the live asset
 * (lobby.getdasha.com/client/simp-board.js, etag 6d1fcc4099abde15) via terser
 * beautify — the on-disk client had fallen behind production (no beatChip,
 * no share push). Live is authoritative; this file is now the source of record.
 */
(function(global) {
  'use strict';
  ...
  var QUIZ_CARDS = {
    'Dasha scholar': {
      ...
      quote: 'It’s time $dasha',
      source: '2085544531739754651'
    },
    'Confirmed simp': {
      image: '/simp/photo/weekend.jpg',
      quote: 'How u crying at the casino and u can’t even get in',
      source: '2085405075686801789'
    },
    'Deep in the lore': {
      image: '/simp/photo/chart.jpg',
      quote: 'It’s time this time',
      source: '2085524407225884699'
    },
    'Watching respectfully': {
      image: '/simp/photo/profile.jpg',
      quote: 'Did you buy my coin',
      source: '2008730208350990657'
    },
    'Dasha curious': {
      image: '/simp/photo/weekend.jpg',
      quote: 'All I want is free healthcare, honey',
      source: '1011745071983296512'
    }
  };
  ...
  function setGateAutoJoin(on) {
    try {
      if (on) sessionStorage.setItem(GATE_AUTOJOIN, '1'); else sessionStorage.removeItem(GATE_AUTOJOIN);
    } catch (e) {}
  }
  ...
  function mount(root, opts) {
    ...
    var retakeBtn = el('button', 'simp-action', 'Retake quiz');
    ...
    var quizBox = el('div', 'simp-quiz-box');
    quizBox.hidden = true;
    ...
    if (homeBoard) {
      ...
      var homeQuiz = existingGo || el('button', 'simp-quiz-go', 'Take Quiz');
      homeQuiz.type = 'button';
      homeQuiz.className = 'simp-quiz-go';
      homeQuiz.textContent = 'Take Quiz';
      homeQuiz.setAttribute('aria-label', 'Start the Dasha simp quiz');
      if (!homeQuiz.getAttribute('data-dasha-take-quiz')) homeQuiz.setAttribute('data-dasha-take-quiz', '');
    ...
    function paintChallengeNote() {
      ...
      // Lane (e.g. "Cinema obsessive") never in note or share — score + title only.
      var bits = [ 'Beat ' + challengeResult.correct + '/' + challengeResult.total ];
      if (challengeResult.title) bits.push(challengeResult.title);
      quizNote.textContent = bits.join(' · ');
    }
    ...
    function setQuizOpen(on) {
      ...
      if (on && root.scrollIntoView) root.scrollIntoView({
        behavior: 'smooth',
        block: 'start'
      });
    }
    ...
    function showQuizChrome() {
      ...
      var quizTitle = quiz.querySelector('.simp-quiz-title');
      if (quizTitle) quizTitle.hidden = false;
    }
    function hideQuizIntro() {
      ...
      var quizTitle = quiz.querySelector('.simp-quiz-title');
      if (quizTitle) quizTitle.hidden = true;
    }
    function closeQuiz() {
      hideResultSticky();
      quizBox.hidden = true;
    ...
    function postJoin() {
      return fetchJson(base + '/simp/join', {
        ...
        if (!res.data || !res.data.ok) {
          return {
            ok: false,
            error: res.data && res.data.error || 'Join failed'
          };
        }
        return {
          ok: true,
    ...
    function challengeKey(entry) {
      ...
      var url = entry && entry.quiz && entry.quiz.resultUrl;
      if (typeof url === 'string') {
        var m = String(url).match(/\/simp\/r\/([A-Za-z0-9_-]{6,20})/);
        if (m) return m[1];
      }
      return '';
    }
    function challengeSeed(entry) {
      if (!entry || entry.kind === 'editorial' || entry.measured === false) return null;
      var q = entry.quiz;
      if (!q || q.title == null || q.correct == null || q.total == null) return null;
      var correct = Number(q.correct);
      var total = Number(q.total);
      if (!isFinite(correct) || !isFinite(total) || total <= 0 || correct < 0 || correct > total) return null;
      var seed = {
        title: String(q.title),
        correct: correct,
        total: total
      };
      if (q.lane) seed.lane = String(q.lane);
      var key = challengeKey(entry);
      if (key) seed.id = key;
      return seed;
    }
    function startChallenge(entry) {
      var seed = challengeSeed(entry);
      if (!seed) return;
      challengeResult = seed;
      if (seed.id) challengeId = seed.id;
      try {
        if (seed.id) {
          var u = new URL(location.href);
          u.searchParams.set('challenge', seed.id);
          if (homeBoard) u.hash = 'simp';
          history.replaceState(null, '', u.pathname + u.search + u.hash);
        }
      } catch (e) {}
      if (homeBoard) mountHomeQuiz();
      paintChallengeNote();
      startQuiz();
    }
    function beatChip(entry) {
      var seed = challengeSeed(entry);
      if (!seed) return null;
      var key = seed.id || '';
      var label = 'Beat ' + seed.correct + '/' + seed.total;
      var chip = key ? document.createElement('a') : el('button', 'simp-beat', label);
      chip.className = 'simp-beat';
      chip.textContent = label;
      chip.setAttribute('aria-label', label);
      if (key) {
        chip.href = resultShareUrl(key);
      } else {
        chip.type = 'button';
      }
      chip.addEventListener('click', function(ev) {
        ev.preventDefault();
        startChallenge(entry);
      });
      return chip;
    }
    ...
    function rowClean(entry) {
      ...
      if (entry.handle) {
        ...
        r.className = 'simp-handle';
        r.href = entry.href;
        r.textContent = line;
        art.appendChild(r);
        art.appendChild(el('span', 'simp-pts', ''));
      } else {
        art.appendChild(el('span', 'simp-handle', line || handle));
        art.appendChild(el('span', 'simp-pts', ''));
      }
      var chip = beatChip(entry);
      if (chip) art.appendChild(chip);
      return art;
    }
    function paintTypes() {
    ...
    function paintBoard() {
      ...
      (boardData.measured || []).forEach(function(m) {
        rows.push(m);
      });
      if (!rows.length) {
    ...
    function paintMe() {
      ...
      quizBtn.setAttribute('aria-label', quizResult ? 'Share your quiz result' : 'Start the Dasha simp quiz');
    ...
    function copyQuizInvite(btn, url) {
      ...
      function miss() {
        setStatus('Copy did not stick — select and copy the link yourself', 'warn');
      }
      function verify() {
    ...
    function openScrollConnectCard() {
      ...
      var st = document.createElement('style');
      st.textContent = '.simp-x-ask{position:fixed;right:16px;bottom:calc(172px + env(safe-area-inset-bottom,0px));z-index:40;width:min(320px,calc(100vw - 32px));background:#f4eddb;color:#070608;border:3px solid #070608;box-shadow:6px 6px 0 #ff3b81;padding:16px 18px}' + '.simp-x-ask p{margin:0 0 12px;font:700 15px/1.3 Arial,Helvetica,sans-serif}' + '.simp-x-ask .x-go{display:inline-flex;align-items:center;justify-content:center;min-height:48px;padding:0 18px;background:#dfff00;color:#070608;font:900 1rem/1 "Arial Black",Helvetica,sans-serif;text-transform:uppercase;border:3px solid #070608;box-shadow:4px 4px 0 #ff3b81;cursor:pointer}' + '.simp-x-ask .x-skip{display:block;margin-top:8px;background:none;border:0;color:#070608;font:900 1rem/1.2 "Arial Black",Helvetica,sans-serif;text-decoration:underline;cursor:pointer;min-height:48px}';
    ...
    function watchHomeConnectAsk() {
      ...
      if (simp && 'IntersectionObserver' in window) {
        askIo = new IntersectionObserver(function(ents) {
      ...
      if (simp) {
        askScroll = function() {
          var r = simp.getBoundingClientRect();
          if (r.top < window.innerHeight * .72 && r.bottom > 80) fire();
        };
        window.addEventListener('scroll', askScroll, {
          passive: true
        });
      }
    }
    function afterLinkedJoin() {
      ...
      return refresh().then(function() {
        ...
        if (meData.enrolled) setStatus('X linked · on the board', 'ok'); else setStatus('X linked — join the board when ready', 'ok');
      }).catch(function() {
        setGateAutoJoin(false);
      });
    }
    function loadImage(src) {
      return new Promise(function(resolve, reject) {
        ...
        img.onerror = reject;
        img.src = src.charAt(0) === '/' ? base + src : src;
      });
    }
    ...
    function wrapText(ctx, text, x, y, maxWidth, lineHeight) {
      ...
      lines.slice(0, 3).forEach(function(row, index) {
    ...
    function quizCardBlob(result, photoIndex) {
      ...
      return loadImage(QUIZ_PHOTOS[photoIndex % QUIZ_PHOTOS.length]).then(function(img) {
        ...
        cover(ctx, img, 0, 0, 1200, 675);
        var shade = ctx.createLinearGradient(450, 0, 1200, 0);
        shade.addColorStop(0, 'rgba(7,6,8,.12)');
        shade.addColorStop(.58, 'rgba(7,6,8,.88)');
        shade.addColorStop(1, '#070608');
        ctx.fillStyle = shade;
        ctx.fillRect(0, 0, 1200, 675);
        ctx.fillStyle = '#dfff00';
        ctx.font = '900 28px Arial';
        ctx.fillText('DASHA SIMP QUIZ', 650, 76);
        ctx.fillStyle = '#f4eddb';
        ctx.font = '900 58px Arial';
        wrapText(ctx, '“' + card.quote + '”', 650, 158, 490, 66);
        ctx.fillStyle = '#ff3b81';
        ctx.font = '900 112px Arial';
        ctx.fillText(result.correct + '/' + result.total, 646, 450);
        ctx.fillStyle = '#f4eddb';
        ctx.font = '800 26px Arial';
        wrapText(ctx, result.title, 650, 492, 490, 32);
        ctx.font = '700 24px Arial';
        ctx.fillText(meData && meData.x && meData.x.display || '@dash_eats', 650, 553);
        ctx.fillStyle = '#dfff00';
        ctx.font = '800 22px Arial';
        ctx.fillText('getdasha.com', 650, 618);
    ...
    /** Public dare URL — always www, never lobby.* or /?challenge=. */
    function resultShareUrl(idOrUrl) {
      var m = String(idOrUrl || '').match(/(?:^|\/simp\/r\/)([A-Za-z0-9_-]{6,20})(?:$|[/?#])/);
      return m ? 'https://www.getdasha.com/simp/r/' + m[1] : QUIZ_INVITE_URL;
    }
    /** The one share string: "Beat N/M · {title}" / "Beat this" / "$dasha" / URL. */
    function quizShareText(result) {
    ...
    function sendQuizCard(result, blob) {
      ...
      if (blob && navigator.canShare) {
        try {
          var file = new File([ blob ], 'dasha-simp-result.png', {
            type: 'image/png'
          });
    ...
    function resultShareText(result, url) {
      if (result && url) result.resultUrl = url;
      return quizShareText(result);
    }
    ...
    function showSharePush(result, resultUrl) {
      ...
      var title = el('h2', '', result && result.title || 'Result');
      title.id = 'dasha-share-push-title';
      card.appendChild(title);
      var urlLine = el('p', 'simp-share-push-url', resultShareText(result, url));
      urlLine.setAttribute('aria-label', 'Result share text');
      card.appendChild(urlLine);
      var acts = el('div', 'simp-share-push-actions');
      var copyBtn = el('button', '', 'Copy link');
      copyBtn.type = 'button';
      copyBtn.addEventListener('click', function() {
        copyText(resultShareText(result, url), copyBtn, 'Copied');
      });
      ...
      if (navigator.share) {
        ...
        native.addEventListener('click', function() {
          trackQuiz('share');
          navigator.share({
            text: resultShareText(result, url)
          }).catch(function(err) {
    ...
    function finishQuizResult(result, resultUrl) {
      result = result || lastQuizResult;
      if (result && resultUrl) result.resultUrl = resultUrl;
      if (result) return shareQuiz(result);
      return showSharePush(result, resultUrl);
    }
    function shareQuiz(result) {
      ...
      lastQuizResult = result;
      hideSharePush();
      var photoIndex = 0, previewUrl = '', cardBlob = null;
      function paintShareChrome(blob) {
        ...
        retakeBtn.hidden = true;
        setQuizOpen(false);
        quizBox.appendChild(el('h2', 'simp-result-type', result.title || 'Result'));
        if (result.correct != null) {
          quizBox.appendChild(el('p', 'simp-result-score', result.correct + '/' + result.total));
        }
        if (result.vibeNote) {
          quizBox.appendChild(el('p', 'simp-result-vibe', String(result.vibeNote).split('\n')[0]));
        }
        if (blob) {
          ...
          img.className = 'simp-result-card';
        ...
        var copy = el('button', 'simp-action', 'Copy');
        copy.type = 'button';
      ...
      function paintPreview() {
        ...
        quizCardBlob(result, photoIndex).then(function(blob) {
    ...
    function paintLinkedChip() {
      ...
      if (!chip) {
        ...
        if (lobbyPill) nav.insertBefore(chip, lobbyPill); else nav.appendChild(chip);
      }
      var handle = meData.x.display;
      chip.textContent = meData.enrolled ? handle + ' · board' : handle;
      chip.setAttribute('aria-label', meData.enrolled ? handle + ' on Simp Board — open board' : handle + ' linked — open Simp Board');
    ...
    function postQuiz(body) {
      ...
      return fetchJson(base + '/simp/quiz', {
        method: 'POST',
        credentials: 'include',
        mode: 'cors',
        headers: {
    ...
    function renderQuestion(data) {
      ...
      var bar = el('div', 'simp-quiz-bar');
      bar.setAttribute('role', 'progressbar');
      bar.setAttribute('aria-valuenow', String(current));
      if (total) bar.setAttribute('aria-valuemax', String(total));
      var fill = el('span', 'simp-quiz-fill');
      fill.style.width = (total ? Math.min(100, current / total * 100) : 6) + '%';
      bar.appendChild(fill);
      stage.appendChild(bar);
      if (current && total) stage.appendChild(el('p', 'simp-quiz-count', current + ' / ' + total));
      var media = data.question.media || data.question.image && {
      ...
      var choices = el('div', 'simp-quiz-choices');
      choices.setAttribute('role', 'group');
      choices.setAttribute('aria-labelledby', question.id);
      data.question.choices.forEach(function(choice, index) {
        var button = el('button', 'simp-quiz-choice');
        button.type = 'button';
        button.appendChild(el('span', 'simp-quiz-key', String(index + 1)));
        button.appendChild(el('span', '', choice));
    ...
    function renderFeedback(data, after) {
      ...
      if (data.feedback.source) {
        var source = el('a', 'simp-quiz-source', 'Source ↗');
        source.href = data.feedback.source;
      ...
      var advance = function() {
        if (advanced) return;
        advanced = true;
        after ? after() : renderQuestion(data);
      };
      stage.addEventListener('click', function(event) {
        if (!event.target.closest('a')) advance();
      });
      quizBox.appendChild(stage);
      setTimeout(function() {
        if (quizBox.contains(stage)) advance();
      }, hold);
    }
      ...
      postQuiz({
        ...
        if (res.data.done) {
          ...
          return renderFeedback(res.data, function() {
            ...
            setStatus(res.data.retake ? 'Score updated · share your result' : 'Quiz scored · Board joined · share your result', 'ok');
            finishQuizResult(res.data.quiz || lastQuizResult, res.data.resultUrl);
            return refresh();
          });
        }
    ...
    root.addEventListener('keydown', function(event) {
      if (event.key === 'Escape' && root.classList.contains('simp-quiz-active')) {
    ...
    function mountHomeQuiz() {
      if (quiz.parentNode) return;
      quizActions.appendChild(quizBtn);
      quizActions.appendChild(retakeBtn);
      root.insertBefore(quiz, status);
    }
    quizBtn.addEventListener('click', function() {
      ...
      if (quizBtn.dataset.mode === 'share') {
        ...
        return finishQuizResult(shared, shared && shared.resultUrl);
      }
      startQuiz();
    });
    if (homeBoard) {
      homeQuiz.addEventListener('click', function() {
        mountHomeQuiz();
        startQuiz();
      });
    }
    ...
    function refresh() {
      return Promise.all([ fetchJson(base + '/simp/board', {
        method: 'GET',
        mode: 'cors',
        cache: 'no-store'
    ...
    if (challengeId && /^[A-Za-z0-9_-]{6,20}$/.test(challengeId)) fetchJson(base + '/simp/result/' + challengeId, {
      method: 'GET',
      mode: 'cors',
      cache: 'no-store'
    }).then(function(res) {
      if (res.data && res.data.result) {
        challengeResult = res.data.result;
        paintChallengeNote();
      }
    }).catch(function() {});
    ...
    holderBtn.addEventListener('click', function() {
      ...
      wallet.connect().then(function(connected) {
        ...
        return fetchJson(base + '/simp/wallet/challenge', {
          method: 'POST',
          credentials: 'include',
          mode: 'cors',
          headers: {
        ...
        holderBtn.disabled = false;
        setStatus(String(err.message || err).slice(0, 100), 'bad');
      });
    });
    ...
    document.addEventListener('dasha-still-quiz', function() {
      scrollToQuiz();
      startQuiz();
    });
