(function () {
  ...
  function companyRow(company, index) {
    ...
    var links = [];
    if (jobsUrl) links.push('<a href="' + esc(jobsUrl) + '" target="_blank" rel="noopener noreferrer">' + (openRoles ? rolesLabel + ' on ' + esc(company.atsSource) : 'careers') + '</a>');
  ...
  function render(root, map) {
    ...
    root.innerHTML =
      ...
      '<p class="dg-dir-count" role="status" aria-live="polite">' + companies.length + ' companies · ' + hiringNow + ' with live US-posted open roles · ' + reviewed + ' reviewed community submissions</p>' +
