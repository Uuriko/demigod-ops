...
- Exact CA · public source · direct buy route
...
1. Identity is **one mesh**: worker-hosted `/client/dasha.glb` (skin + in-place `step` clip). Same face, hair, body, clothes in every frame. Playback is Three.js `0.170.0` `GLTFLoader` + `AnimationMixer` (same pin as `/graph`). Clip root stays at origin. Budget under ~1.5MB. POT atlas + mipmaps. No Draco required at this size.
2. **Toon-keyed, not a photoreal PBR doll.** `MeshToonMaterial` with 2–4 light bands. Transparent clear (no studio floor, no HDRI). One key + one acid `#dfff00` rim, quantized on/off (`step`). No bloom, SSAO, or post stack. Face albedo is the real wiki-2022 crop (moles stay); cotton-2014 supplies hair. Body is 3–5 big shapes (black tee, SCARY cap, big hair) so the silhouette reads at ~120px. Face is the only detail.
3. Locked ortho, slight 3/4. No perspective stretch as she crosses. Client drives full-width X ping-pong and a real yaw turn. Every few crossings: short look-at-camera beat. `visibilitychange` cancels rAF and resets `THREE.Clock` / `lastTime` on return. `IntersectionObserver` pauses off-screen. Motion is delta-time.
4. Speaker is a **fixed 48×48 DOM button on the right** (waves die when muted). Acid `#dfff00` on ink `#070608`. `aria-label` Mute/Unmute, focus-visible acid outline. Canvas is decorative. iOS: `AudioContext.resume` + `play()` on first pointer. Click speaker or dancer. Persist `localStorage.dashaMute`. Same Umplix CC0 loop.
5. Dock is the one 3D object on home. Hidden on `/graph`. Reduced-motion / no-WebGL / low GPU (`failIfMajorPerformanceCaveat`): still pose, no travel, no music (`/client/dasha-face.webp`). Home first paint stays headline + Buy $dasha; Three + GLB idle-boot after first paint. `pixelRatio` cap 2.
...
**Politics:** Public record includes Sanders → Trump votes, controversy around platforming far-right guests (2025 agency drop). Product site **does not campaign**. Keep getdasha about Dasha, the Studio, community, and the exact mint.
...
| Date | Method | Notes |
|------|--------|-------|
| 2026-08-06 | X scrape → `dasha-desk/docs/X-RESEARCH-DASHA-2026-08-06.md` | Quotes, media IDs, Perry tape |
