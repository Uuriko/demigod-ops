...
const CLI = path.join(OPS, 'demigod-heavy-competitors-handoff.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/competitors-brief';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'competitors-brief');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-COMPETITORS-BRIEF.json',
  '/home/potter/HEAVY-COMPETITORS-FEATURES-REPLY.md',
  '/home/potter/HEAVY-COMPETITORS-FEATURES-SENT.txt',
  path.join(OPS, 'DEMIGOD-HEAVY-COMPETITORS-BRIEF.json'),
  path.join(OPS, 'HEAVY-COMPETITORS-FEATURES-REPLY.md'),
  path.join(OPS, 'HEAVY-COMPETITORS-FEATURES-SENT.txt'),
  path.join(OPS, 'DEMIGOD-HEAVY-COMPETITORS-HANDOFF.json'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-competitors-handoff') || name.startsWith('HEAVY-COMPETITORS-FEATURES')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-COMPETITORS-BRIEF.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-COMPETITORS-FEATURES-REPORT.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'COMPETITOR notes stay local.',
    'FEATURE matrix stays on disk.',
    'ARCHITECTURE fits the foot.',
    'Fonzi is a seed name.',
    'Paraform is a seed name.',
    'Jill is a seed name.',
  ].join('\n');
}

describe('competitors brief', { concurrency: 1 }, () => {
  test('a competitors brief stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-brief-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-brief-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-brief-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-brief-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-brief-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-brief-west-'));
    ...
    const outsideFile = path.join(outside, 'brief.md');
    fs.writeFileSync(outsideFile, 'Outside keep secret\nCOMPETITOR FEATURE ARCHITECTURE Fonzi Paraform Jill\n');
    ...
    assert.equal(unsetOut.error, 'brief_root_required');
    ...
    assert.equal(homeOut.error, 'brief_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-COMPETITORS-BRIEF.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-COMPETITORS-FEATURES-REPLY.md'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-COMPETITORS-FEATURES-SENT.txt'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'brief_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-COMPETITORS-FEATURES-REPLY.md')), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-COMPETITORS-FEATURES-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-COMPETITORS-FEATURES-REPORT.md'));
    const linked = run(leak, [], {}, cwdDir);
    const linkedOut = readJson(linked, 'symlink brief');
    ...
    plant(bad, 'Harbor Bad keep', 'COMPETITOR notes only.');
    const badRun = run(bad, [], {}, cwdDir);
    assert.equal(badRun.status, 1, `thin brief should fail\n${badRun.stdout}\n${badRun.stderr}`);
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-COMPETITORS-FEATURES-REPLY.md')), false);
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-COMPETITORS-FEATURES-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(bad, 'DEMIGOD-HEAVY-COMPETITORS-HANDOFF.json')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('FEATURE'), true);
    assert.equal(badStored.missing.includes('ARCHITECTURE'), true);
    assert.equal(badStored.missing.includes('Fonzi'), true);
    assert.equal(badStored.missing.includes('Paraform'), true);
    assert.equal(badStored.missing.includes('Jill'), true);

    plant(east, 'Harbor East keep', goodBody());
    const eastRun = run(east, [], {}, cwdDir);
    assert.equal(eastRun.status, 0, `east brief failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-COMPETITORS-FEATURES-REPLY.md')), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-COMPETITORS-FEATURES-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(east, 'DEMIGOD-HEAVY-COMPETITORS-HANDOFF.json')), false);
    ...
    assert.equal(stored.markers.COMPETITOR, true);
    assert.equal(stored.markers.FEATURE, true);
    assert.equal(stored.markers.ARCHITECTURE, true);
    assert.equal(stored.markers.Fonzi, true);
    assert.equal(stored.markers.Paraform, true);
    assert.equal(stored.markers.Jill, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'competitors-brief', 'brief.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west brief failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-COMPETITORS-FEATURES-REPLY.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-COMPETITORS-FEATURES-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'competitors-brief', 'brief.shot')), false);
