...
const graphMessage = walletMessage({ handle: 'ava', publicKey: proofAddress, nonce: 'abcdefgh', issuedAt: now, expiresAt: now + 1000, requestId: 'graph-highlight' });
...
assert.match(faucetMessage, /Dest-proof for the \/faucet sample/);
assert.match(faucetMessage, /No transaction/);
assert.doesNotMatch(faucetMessage, /we will not ask for a phrase|not an airdrop|claim-airdrop|not earn|not the dev|association is not endorsement|neither is required|not official|not advice/i);
