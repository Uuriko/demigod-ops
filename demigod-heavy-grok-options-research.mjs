#!/usr/bin/env node
/**
 * Local options-research check in an explicit data root.
 * Writes DEMIGOD-GROK-BUILD-OPTIONS-RESEARCH.json under DEMIGOD_ROOT. Does not send a prompt.
 */
import fs from 'fs';
...
const MARKERS = [
  { name: 'OPTION', re: /option/i },
  { name: 'ROUTING', re: /routing/i },
  { name: 'RULE', re: /rule/i },
  { name: 'PROJECT', re: /project/i },
  { name: 'BUILD', re: /build/i },
];
...
function reportPath() {
  return path.join(dataRoot(), 'DEMIGOD-GROK-BUILD-OPTIONS-RESEARCH.json');
}
function shotDir() {
  return path.join(dataRoot(), 'audit-shots', 'grok-build-options');
}
...
function main() {
  ...
  if (!root || path.resolve(root) === '/home/potter' || path.resolve(root) === path.resolve(scriptDir())) {
    refuse('options_root_required');
  }
  const footText = readText(root, 'demigod-foot-core.js');
  const notes = readText(root, 'HEAVY-GROK-BUILD-OPTIONS-SOURCE.md');
  ...
  const dir = shotDir();
  const shot = path.join(dir, 'options.shot');
