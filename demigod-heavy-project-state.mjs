#!/usr/bin/env node
/**
 * Local state check in an explicit data root.
 * Writes DEMIGOD-HEAVY-PROJECT-STATE.json under DEMIGOD_ROOT. Does not spawn a child or send a prompt.
 */
import fs from 'fs';
...
function reportPath() {
  return path.join(dataRoot(), 'DEMIGOD-HEAVY-PROJECT-STATE.json');
}
function shotDir() {
  return path.join(dataRoot(), 'audit-shots', 'project-state');
}
...
function main() {
  ...
  if (!root || path.resolve(root) === '/home/potter' || path.resolve(root) === path.resolve(scriptDir())) {
    refuse('state_root_required');
  }
  const footText = readText(root, 'demigod-foot-core.js');
  const notes = readText(root, 'HEAVY-PROJECT-STATE-SOURCE.md');
  ...
  const dir = shotDir();
  const shot = path.join(dir, 'state.shot');
