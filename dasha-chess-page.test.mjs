...
assert.equal((source.match(/href="https:\/\/www\.getdasha\.com\/"/g) || []).length, 3, 'Chess brand, Home, and slim footer must cross the lobby subdomain back to the canonical homepage');
assert.doesNotMatch(source, /class="(?:brand|back)" href="\/"/, 'Chess navigation must not mislabel the lobby service root as Home');
assert.match(source, /<a href="https:\/\/www\.getdasha\.com\/">\$dasha<\/a> · <a href="\/chess">Chess<\/a> · <a class="buy-dasha"/);
assert.doesNotMatch(source.match(/<footer[\s\S]*?<\/footer>/i)?.[0] || '', /\/learn|\/faucet|\/airdrop|\/earn|\/bounties|\/privacy|\/verse|\/graph/, 'chess footer must not be a sitemap');
