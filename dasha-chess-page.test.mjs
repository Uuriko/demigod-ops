...
assert.match(source, /class="dasha-ham"/);
assert.match(source, /href="\/graph">Graph</);
assert.match(source, /href="\/verse">Verse</);
assert.match(source, /@media\(max-width:560px\)\{[\s\S]*?\.dasha-ham\{flex-wrap:wrap/, 'narrow chess header must wrap');
assert.doesNotMatch(source.match(/<nav aria-label="Dasha"[\s\S]*?<\/nav>/)?.[0] || '', /\/bounties|\/learn|\/faucet|\/forum|\/rally|\/privacy/, 'chess hamburger must stay bounded');
...
assert.doesNotMatch(source.match(/<footer[\s\S]*?<\/footer>/i)?.[0] || '', /\/learn|\/faucet|\/airdrop|\/earn|\/bounties|\/privacy|\/verse|\/graph|\/chess/, 'chess footer must not be a sitemap');
