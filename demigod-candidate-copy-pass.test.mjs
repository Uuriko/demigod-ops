...
const CLI = path.join(OPS, 'demigod-candidate-copy-pass.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/candidate-copy';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'candidate-copy');
const SHARED = [
  '/home/potter/DEMIGOD-CANDIDATE-COPY-PASS.json',
  path.join(OPS, 'DEMIGOD-CANDIDATE-COPY-PASS.json'),
  path.join(OPS, 'DEMIGOD-COPY-SPEC.json'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-candidate-copy') || name.startsWith('DEMIGOD-CANDIDATE-COPY')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-CANDIDATE-COPY-PASS.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-CANDIDATE-COPY-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'CANDIDATE notes stay local.',
    'COPY stays on disk.',
    'HERO stays planted.',
    'FORM stays local.',
...
describe('candidate copy pass', { concurrency: 1 }, () => {
  test('a copy record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-ccp-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-ccp-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-ccp-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-ccp-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-ccp-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-ccp-west-'));
    ...
    assert.equal(src.includes('cdp-close-tabs'), false);
    ...
    assert.equal(src.includes('demigod-fix-custom-code'), false);
    ...
    assert.equal(unsetOut.error, 'candidate_root_required');
    ...
    assert.equal(homeOut.error, 'candidate_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-CANDIDATE-COPY-PASS.json'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'candidate_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'DEMIGOD-COPY-SPEC.json')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    const live = run(east, ['--ai'], {}, cwdDir);
    const liveOut = readJson(live, 'ai');
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-CANDIDATE-COPY-SOURCE.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'CANDIDATE notes only.');
    ...
    assert.equal(badStored.missing.includes('COPY'), true);
    assert.equal(badStored.missing.includes('HERO'), true);
    ...
    assert.equal(stored.markers.CANDIDATE, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'candidate-copy', 'copy.shot'), 'utf8');
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'DEMIGOD-COPY-SPEC.json')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'candidate-copy', 'copy.shot')), false);
