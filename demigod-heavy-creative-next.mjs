#!/usr/bin/env node
/**
 * Local creative-next check in an explicit data root.
 * Writes DEMIGOD-HEAVY-CREATIVE-NEXT.json under DEMIGOD_ROOT. Does not send a prompt.
 */
import fs from 'fs';
...
function reportPath() {
  return path.join(dataRoot(), 'DEMIGOD-HEAVY-CREATIVE-NEXT.json');
}
function shotDir() {
  return path.join(dataRoot(), 'audit-shots', 'creative-next');
}
...
function main() {
  ...
  if (!root || path.resolve(root) === '/home/potter' || path.resolve(root) === path.resolve(scriptDir())) {
    refuse('creative_root_required');
  }
  const footText = readText(root, 'demigod-foot-core.js');
  const status = readText(root, 'DEMIGOD-LEVERAGE-STATUS.json');
  if (footText == null && status == null) refuse('source_required');
  const text = status || '';
  const hits = markerHits(text);
  const missing = MARKERS.filter((marker) => !hits[marker.name]).map((marker) => marker.name);
  const pass = status != null && missing.length === 0;
