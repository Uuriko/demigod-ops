...
const CLI = path.join(OPS, 'demigod-form-e2e.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/form-e2e';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'form-e2e');
const SHARED = [
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-form-e2e') || name.startsWith('DEMIGOD-FORM-E2E')).sort();
}
...
function plant(dir, phrase, body) {
  ...
  fs.writeFileSync(path.join(dir, 'HEAVY-FORM-E2E-SOURCE.md'), `${phrase}\n${body}\n`);
}
...
describe('form e2e', { concurrency: 1 }, () => {
  test('a form record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-fe2-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-fe2-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-fe2-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-fe2-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-fe2-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-fe2-west-'));
    ...
    assert.equal(src.includes('formspree'), false);
    ...
    assert.equal(src.includes("'/tmp'"), false);
    ...
    const dry = run(east, ['--dry'], {}, cwdDir);
    const dryOut = readJson(dry, 'dry');
    assert.equal(dry.status, 1);
    assert.equal(dryOut.error, 'publish_refused');
    assert.equal(dryOut.liveFetch, false);
    assert.equal(dryOut.sent, false);
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal((dry.stdout + dry.stderr).includes('https://'), false);
    assert.equal((dry.stdout + dry.stderr).includes('trydemigod.com'), false);

    const submit = run(east, ['--submit'], {}, cwdDir);
    const submitOut = readJson(submit, 'submit');
    assert.equal(submit.status, 1);
    assert.equal(submitOut.error, 'publish_refused');
    assert.equal(submitOut.sent, false);
    assert.equal(submitOut.liveFetch, false);
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal((submit.stdout + submit.stderr).includes('webflow'), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-FORM-E2E-SOURCE.md'));
    ...
    assert.equal(badStored.missing.includes('E2E'), true);
    assert.equal(badStored.missing.includes('HIRE'), true);
    ...
    assert.equal(eastRun.status, 0, `east form failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.E2E, true);
    assert.equal(stored.markers.HIRE, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'form-e2e', 'e2e.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west form failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'form-e2e', 'e2e.shot')), false);
