...
function introDir() {
  return path.join(ROOT, 'DEMIGOD-INTROS');
}

/** Both yes flags must name the same shortlisted person. Latest yes on each side counts. */
function introState(p) {
  const m = p.mutual || {};
  const agreedId = m.founderYesFor && m.founderYesFor === m.candidateYesFor ? m.founderYesFor : '';
  const cand = agreedId ? (p.shortlist || []).find((row) => row.id === agreedId) || null : null;
  const agreed = Boolean(cand);
  const ready = agreed
    && cand.consent === true
    && Boolean(p.outcome90d)
    && (p.status === 'shortlist' || p.status === 'intro');
  return { m, cand, agreed, ready };
}
...
if (cmd === 'yes') {
  ...
  if (p.mutual.founderYesFor && p.mutual.founderYesFor === p.mutual.candidateYesFor) {
    p.mutual.candId = p.mutual.founderYesFor;
  } else {
    delete p.mutual.candId;
  }
  save(data);
...
if (cmd === 'packet' || cmd === 'send') {
  ...
  const { m, cand, ready } = introState(p);
  if (!ready) {
    console.error(
      JSON.stringify({
        ok: false,
  ...
  console.log(JSON.stringify({ ok: true, packet: out, status: p.status, mutual: m, ready, sent: false, liveMail: false }, null, 2));
  process.exit(0);
}
