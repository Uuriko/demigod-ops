#!/usr/bin/env node
/**
 * Local session handoff check in an explicit data root.
 * Writes DEMIGOD-HEAVY-NEXT-RESEARCH.json under DEMIGOD_ROOT. Does not spawn a child or send a prompt.
 */
import fs from 'fs';
...
function reportPath() {
  return path.join(dataRoot(), 'DEMIGOD-HEAVY-NEXT-RESEARCH.json');
}
function shotDir() {
  return path.join(dataRoot(), 'audit-shots', 'session-handoff');
}
...
function main() {
  ...
  if (!root || path.resolve(root) === '/home/potter' || path.resolve(root) === path.resolve(scriptDir())) {
    refuse('session_root_required');
  }
  const footText = readText(root, 'demigod-foot-core.js');
  const notes = readText(root, 'HEAVY-SESSION-HANDOFF-SOURCE.md');
