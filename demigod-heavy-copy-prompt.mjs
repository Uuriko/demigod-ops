#!/usr/bin/env node
/**
 * Local copy spec check in an explicit data root.
 * Writes DEMIGOD-HEAVY-COPY.json under DEMIGOD_ROOT. Does not send a prompt.
 */
import fs from 'fs';
...
function reportPath() {
  return path.join(dataRoot(), 'DEMIGOD-HEAVY-COPY.json');
}
function shotDir() {
  return path.join(dataRoot(), 'audit-shots', 'heavy-copy-spec');
}
...
function main() {
  ...
  if (!root || path.resolve(root) === '/home/potter' || path.resolve(root) === path.resolve(scriptDir())) {
    refuse('copy_spec_root_required');
  }
  const footText = readText(root, 'demigod-foot-core.js');
  const brief = readText(root, 'HEAVY-COPY-BRIEF.md');
  ...
  const dir = shotDir();
  const shot = path.join(dir, 'spec.shot');
