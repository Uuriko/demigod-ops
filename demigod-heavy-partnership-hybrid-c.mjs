#!/usr/bin/env node
/**
 * Local partnership check in an explicit data root.
 * Writes DEMIGOD-PARTNERSHIP-HYBRID-C.json under DEMIGOD_ROOT. Does not open a browser or send a prompt.
 */
import fs from 'fs';
...
function reportPath() {
  return path.join(dataRoot(), 'DEMIGOD-PARTNERSHIP-HYBRID-C.json');
}
function shotDir() {
  return path.join(dataRoot(), 'audit-shots', 'partnership-hybrid');
}
...
function main() {
  ...
  if (!root || path.resolve(root) === '/home/potter' || path.resolve(root) === path.resolve(scriptDir())) {
    refuse('hybrid_root_required');
  }
  const footText = readText(root, 'demigod-foot-core.js');
  const notes = readText(root, 'HEAVY-PARTNERSHIP-HYBRID-SOURCE.md');
  ...
  const dir = shotDir();
  const shot = path.join(dir, 'hybrid.shot');
