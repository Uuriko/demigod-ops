#!/usr/bin/env node
/**
 * Local program check in an explicit data root.
 * Writes DEMIGOD-PARTNERSHIP-PROGRAM.json under DEMIGOD_ROOT. Does not open a browser or send a prompt.
 */
import fs from 'fs';
...
const MARKERS = [
  { name: 'PROGRAM', re: /program/i },
  { name: 'PARTNER', re: /partner/i },
  { name: 'REFERRAL', re: /referral/i },
  { name: 'CREDIT', re: /credit/i },
...
function reportPath() {
  return path.join(dataRoot(), 'DEMIGOD-PARTNERSHIP-PROGRAM.json');
}
function shotDir() {
  return path.join(dataRoot(), 'audit-shots', 'partnership-program');
}
...
function main() {
  ...
  if (!root || path.resolve(root) === '/home/potter' || path.resolve(root) === path.resolve(scriptDir())) {
    refuse('program_root_required');
  }
  const footText = readText(root, 'demigod-foot-core.js');
  const notes = readText(root, 'HEAVY-PARTNERSHIP-PROGRAM-SOURCE.md');
  ...
  const dir = shotDir();
  const shot = path.join(dir, 'program.shot');
