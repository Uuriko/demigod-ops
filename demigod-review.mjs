#!/usr/bin/env node
/**
 * demigod-review v2.2 — code review + bugfix orchestrator
 *
...
const KNOWN = new Set([
  ...
  '--print-fix-prompt', '--config', '--no-config', '--changed',
]);
...
function printHelp() {
  console.log(`demigod-review v2.2 — Demigod static review + tier-A fix + gates
...
if (cli.version) {
  console.log('demigod-review 2.2.0');
  process.exit(0);
}
...
if (flags.catalog) {
  ...
  if (flags.json || flags.format === 'json') console.log(JSON.stringify({ version: '2.2.0', rules: cat }, null, 2));
  else {
    console.log('demigod-review rule catalog v2.2');
...
async function main() {
  ...
  if (flags.format === 'json' || flags.json) {
    ...
    console.log(
      `demigod-review v2.2 ${summary.fail ? 'FAIL' : 'OK'} · ${scope.length} files · ${summary.count} findings (${summary.suppressed} suppressed) · failOn=${summary.failOn} · ${report.timing.ms}ms`,
    );
