...
| **T4 — Simp Points manipulation** | Fixed event lanes, one quiz attempt per X and quiz answers scored only by the Worker; holder balances score zero | Schema rejects unknown/public-evidence fields; scorer fixtures cover replay, invalid answers and abuse | `npm run dasha:test:simp` |
