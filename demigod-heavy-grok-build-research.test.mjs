...
const CLI = path.join(OPS, 'demigod-heavy-grok-build-research.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/grok-build-research';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'grok-build-research');
const SHARED = [
  '/home/potter/DEMIGOD-GROK-BUILD-RESEARCH.json',
  '/home/potter/HEAVY-GROK-BUILD-RESEARCH.md',
  '/home/potter/HEAVY-GROK-BUILD-MASTER-PROMPT.md',
  '/home/potter/HEAVY-GROK-BUILD-RESEARCH-SENT.txt',
  path.join(OPS, 'DEMIGOD-GROK-BUILD-RESEARCH.json'),
  path.join(OPS, 'HEAVY-GROK-BUILD-RESEARCH.md'),
  path.join(OPS, 'HEAVY-GROK-BUILD-MASTER-PROMPT.md'),
  path.join(OPS, 'HEAVY-GROK-BUILD-RESEARCH-SENT.txt'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-grok-build') || name.startsWith('HEAVY-GROK-BUILD')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-GROK-BUILD-RESEARCH.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-GROK-BUILD-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'RESEARCH notes stay local.',
    'MASTER prompt stays on disk.',
    'ROLE split stays planted.',
    'RULE list stays local.',
    'SESSION boot stays local.',
  ].join('\n');
}

describe('grok build research', { concurrency: 1 }, () => {
  test('a research record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-gbuild-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-gbuild-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-gbuild-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-gbuild-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-gbuild-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-gbuild-west-'));
    ...
    assert.equal(unsetOut.error, 'build_research_root_required');
    ...
    assert.equal(homeOut.error, 'build_research_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-GROK-BUILD-RESEARCH.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-GROK-BUILD-RESEARCH.md'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-GROK-BUILD-MASTER-PROMPT.md'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-GROK-BUILD-RESEARCH-SENT.txt'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'build_research_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-GROK-BUILD-RESEARCH.md')), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-GROK-BUILD-MASTER-PROMPT.md')), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-GROK-BUILD-RESEARCH-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-GROK-BUILD-SOURCE.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'RESEARCH notes only.');
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-GROK-BUILD-RESEARCH.md')), false);
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-GROK-BUILD-MASTER-PROMPT.md')), false);
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-GROK-BUILD-RESEARCH-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('MASTER'), true);
    assert.equal(badStored.missing.includes('ROLE'), true);
    assert.equal(badStored.missing.includes('RULE'), true);
    assert.equal(badStored.missing.includes('SESSION'), true);

    plant(east, 'Harbor East keep', goodBody());
    const eastRun = run(east, [], {}, cwdDir);
    assert.equal(eastRun.status, 0, `east research failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.RESEARCH, true);
    assert.equal(stored.markers.MASTER, true);
    assert.equal(stored.markers.ROLE, true);
    assert.equal(stored.markers.RULE, true);
    assert.equal(stored.markers.SESSION, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'grok-build-research', 'research.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west research failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-GROK-BUILD-RESEARCH.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-GROK-BUILD-MASTER-PROMPT.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-GROK-BUILD-RESEARCH-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'grok-build-research', 'research.shot')), false);
