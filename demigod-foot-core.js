 ...
 */
window.dgFootVersion = 'v207'; console.log('[demigod] foot v207-core loaded');
(function(){
...
var WIZ_THANKS={startup:{head:'Brief received',lead:'A human will review it — not a bot. Matching is mutual-yes only.',steps:['We read your brief, stack, and 90-day outcome','We propose 3–5 people only when fit is strong','Both sides say yes before any intro'],done:'How it works'},engineer:{head:'Profile saved',lead:'Your resume stays private until a human sees a real fit — then you still say yes.',steps:['Your profile is stored securely — never blasted','A human proposes the match to both sides if fit','hello@trydemigod.com may reach out with one curated intro'],done:'How it works'},partner:{head:'Application received',lead:'We verify partner fit before sending your tracking code.',steps:['A human reviews your partner application','hello@trydemigod.com sends your unique partner code if approved','Warm intros only — we decline bad matches to protect your reputation'],done:'Back to partners'}};
/* ==== SECTION: WIZ_CFG (stepper paths) ==== */
var WIZ_CFG={startup:{steps:[['welcome'],['contact-email'],['company-name'],['company-stage'],['role-title'],['stack-needs'],['90day-outcome'],['salary-range'],['timeline'],['team-size'],['why-this-role'],['role-jd'],['__submit__'],['__thanks__']],welcome:{t:'Hire for your startup',b:'About 2 minutes. One question at a time. A human reads every brief and proposes only strong fits — anchored on the 90-day outcome.',btn:'Start brief →'},thanks:STARTUP_OK,total:11,optional:['phone','salary-range','why-this-role','role-jd','timeline','team-size']},engineer:{steps:[['welcome'],['full-name'],['seeker-email'],['linkedin-url'],['skills-stack'],['experience'],['sf-bay'],['availability'],['salary-expectation'],['why-startups'],['links'],['phone'],['resume'],['__submit__'],['__thanks__']],welcome:{t:'Join the network',b:'About 2 minutes. One profile. Private until a human sees a real fit — then you still say yes.',btn:'Create profile →'},thanks:ENGINEER_OK,total:13,optional:['phone','links','resume','salary-expectation','why-startups','availability']},partner:{steps:[['welcome'],['partner-type'],['partner-name'],['partner-email'],['partner-phone'],['partner-org'],['referral-plan'],['partner-linkedin'],['partner-notes'],['__submit__'],['__thanks__']],welcome:{t:'Partner with Demigod',b:'Refer startups or candidates. Earn when a placement succeeds.',btn:'Apply to partner →'},thanks:PARTNER_OK,total:9,optional:['partner-phone','partner-linkedin','partner-notes']}};
...
function faqBlock(){/* v207: homepage FAQ removed for clarity */ var el=q('#dg-faq'); if(el)el.remove(); var ld=q('#dg-faq-jsonld'); if(ld)ld.remove(); }
...
function proofStrip(){/* v207: proof essay removed */ var el=q('#dg-proof-strip'); if(el)el.remove(); }
function contactStrip(){/* v207: no hero essay — CTAs only */ var el=q('#dg-contact-strip'); if(el)el.remove(); }
...
function closePage() {
  ...
  try {
    if (history.state && history.state.dgPage) {
...
function boot(){if(!document.body)return;forceMainVisible();run();finalButtonLabels();deepLink();try{if(window.requestIdleCallback)requestIdleCallback(function(){try{orgJsonLd()}catch(e){}});else setTimeout(function(){try{orgJsonLd()}catch(e){}},1200)}catch(e){}}boot();document.addEventListener('DOMContentLoaded',boot);[400,1500].forEach(function(ms){setTimeout(boot,ms)});/* v190: drop t=50 forceMainVisible — boot/run cover */
...
window.__dgFootVer='207';console.log('Demigod v207');
