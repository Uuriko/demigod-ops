...
| **C15 LP_CLAIM_OUTSTANDING** | Finalized on-chain observation, verified across three slots on 2026-08-18: the Raydium LP mint `8GDvsE3Nbi…h3Aj` has supply `0` while pool `9KkDpvUQ…7bU7` (owned by Raydium AMM v4) holds reserves. Owner: `dasha-token-observe.mjs`, which records LP supply every run | The observation itself: no LP tokens exist, so no one holds a claim to withdraw the liquidity currently in the pool | That liquidity cannot fall — it moves with every trade and with price; that the pool is permanent — the LP mint authority is Raydium's and is live, so liquidity added later mints new LP its depositor could withdraw; “locked”, “safe”, “rug-proof”; that the project performed the burn or controls the pool |
