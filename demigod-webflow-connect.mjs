...
import { openUrl, listPages, DESIGNER } from './demigod-webflow-lib.mjs';
...
if (!['status', 'bridge', 'all'].includes(cmd) || args.slice(1).some((arg) => arg !== '--json') || args.filter((arg) => arg === '--json').length > 1) {
  console.error('usage: demigod-webflow-connect.mjs status|bridge|all [--json]');
  process.exit(2);
}
...
function mcpCreds() {
  try {
    ...
    const wf = d['webflow:https://mcp.webflow.com/mcp'];
    if (!wf?.token_response?.access_token) return { ok: false, reason: 'no grok webflow token' };
...
async function buildConnectStatus() {
  ...
  const layers = {
    mcpConfigAllAgents: Object.values(configs).every(Boolean),
  ...
  return {
    ...
    tips: [
      ...
      !designerOpen && `Open Bridge: bin/dg-webflow connect bridge`,
...
async function main() {
  ...
  if (asJson || cmd === 'status' || cmd === 'all') {
    if (asJson) console.log(JSON.stringify(status, null, 2));
    else {
      console.log(`# Webflow agent connect · ${status.site}`);
