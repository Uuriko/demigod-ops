...
const CLI = path.join(OPS, 'demigod-heavy-form-fields.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/form-fields';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'form-fields');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-FORM-FIELDS.json',
  '/home/potter/HEAVY-FORM-FIELDS-RESEARCH.md',
  path.join(OPS, 'DEMIGOD-HEAVY-FORM-FIELDS.json'),
  path.join(OPS, 'HEAVY-FORM-FIELDS-RESEARCH.md'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-form-fields') || name.startsWith('HEAVY-FORM-FIELDS')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-FORM-FIELDS.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-FORM-FIELDS-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'FORM notes stay local.',
    'STARTUP side stays on disk.',
    'ENGINEER side stays on disk.',
    'REQUIRED flags stay planted.',
    'FIELD labels stay local.',
  ].join('\n');
}

describe('form fields', { concurrency: 1 }, () => {
  test('a form-fields check stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-fields-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-fields-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-fields-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-fields-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-fields-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-fields-west-'));
    ...
    assert.equal(unsetOut.error, 'fields_root_required');
    ...
    assert.equal(homeOut.error, 'fields_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-FORM-FIELDS.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-FORM-FIELDS-RESEARCH.md'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'fields_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-FORM-FIELDS-RESEARCH.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-FORM-FIELDS-SOURCE.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'FORM notes only.');
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-FORM-FIELDS-RESEARCH.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('STARTUP'), true);
    assert.equal(badStored.missing.includes('ENGINEER'), true);
    assert.equal(badStored.missing.includes('REQUIRED'), true);
    assert.equal(badStored.missing.includes('FIELD'), true);
    ...
    assert.equal(stored.markers.ENGINEER, true);
    assert.equal(stored.markers.REQUIRED, true);
    assert.equal(stored.markers.FIELD, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'form-fields', 'fields.shot'), 'utf8');
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-FORM-FIELDS-RESEARCH.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'form-fields', 'fields.shot')), false);
