...
const CLI = path.join(OPS, 'demigod-hero-canvas-cleanup.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/hero-canvas-cleanup';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'hero-canvas-cleanup');
const SHARED = [
  '/home/potter/DEMIGOD-HERO-CANVAS-CLEANUP.json',
  path.join(OPS, 'DEMIGOD-HERO-CANVAS-CLEANUP.json'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-hero-canvas') || name.startsWith('DEMIGOD-HERO-CANVAS')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HERO-CANVAS-CLEANUP.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-HERO-CANVAS-CLEANUP-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'HERO notes stay local.',
    'CANVAS stays on disk.',
    'CLEANUP stays planted.',
    'MATCH stays local.',
    'LOCAL check stays local.',
  ].join('\n');
}

describe('hero canvas cleanup', { concurrency: 1 }, () => {
  test('a hero record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-hcc-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-hcc-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-hcc-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-hcc-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-hcc-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-hcc-west-'));
    ...
    assert.equal(unsetOut.error, 'hero_root_required');
    ...
    assert.equal(homeOut.error, 'hero_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HERO-CANVAS-CLEANUP.json'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'hero_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(designerOut.aiSubmit, false);
    assert.equal(fs.existsSync(reportFile(east)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal((ai.stdout + ai.stderr).includes('talentlink'), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal((verify.stdout + verify.stderr).includes('trydemigod.com'), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-HERO-CANVAS-CLEANUP-SOURCE.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'HERO notes only.');
    ...
    assert.equal(badStored.missing.includes('CLEANUP'), true);
    assert.equal(badStored.missing.includes('MATCH'), true);
    assert.equal(badStored.missing.includes('LOCAL'), true);

    plant(east, 'Harbor East keep', goodBody());
    const eastRun = run(east, [], {}, cwdDir);
    assert.equal(eastRun.status, 0, `east hero failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.HERO, true);
    assert.equal(stored.markers.CANVAS, true);
    assert.equal(stored.markers.CLEANUP, true);
    assert.equal(stored.markers.MATCH, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'hero-canvas-cleanup', 'hero.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west hero failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'hero-canvas-cleanup', 'hero.shot')), false);
