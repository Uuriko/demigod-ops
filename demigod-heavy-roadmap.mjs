#!/usr/bin/env node
/**
 * Local roadmap check in an explicit data root.
 * Writes DEMIGOD-HEAVY-ROADMAP.json under DEMIGOD_ROOT. Does not open a browser or send a prompt.
 */
import fs from 'fs';
...
const MARKERS = [
  { name: 'ROADMAP', re: /roadmap/i },
  { name: 'FINISH', re: /finish/i },
  { name: 'START', re: /start/i },
  { name: 'PLAN', re: /plan/i },
...
function reportPath() {
  return path.join(dataRoot(), 'DEMIGOD-HEAVY-ROADMAP.json');
}
function shotDir() {
  return path.join(dataRoot(), 'audit-shots', 'roadmap');
}
...
function main() {
  ...
  if (!root || path.resolve(root) === '/home/potter' || path.resolve(root) === path.resolve(scriptDir())) {
    refuse('roadmap_root_required');
  }
  const footText = readText(root, 'demigod-foot-core.js');
  const notes = readText(root, 'HEAVY-CURSOR-ROADMAP-BRIEF.md');
  ...
  const dir = shotDir();
  const shot = path.join(dir, 'roadmap.shot');
