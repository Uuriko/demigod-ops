...
const CLI = path.join(OPS, 'demigod-heavy-pipeline.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/pipeline';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'pipeline');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-PIPELINE.json',
  '/home/potter/HEAVY-PIPELINE-PLAN.md',
  path.join(OPS, 'DEMIGOD-HEAVY-PIPELINE.json'),
  path.join(OPS, 'HEAVY-PIPELINE-PLAN.md'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-pipeline') || name.startsWith('DEMIGOD-HEAVY-PIPELINE') || name.startsWith('HEAVY-PIPELINE-PLAN')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-PIPELINE.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-PIPELINE-BRIEF.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'PIPELINE notes stay local.',
    'BOARD stays on disk.',
    'FORM stays planted.',
    'SUBMISSION stays local.',
    'LOCAL check stays local.',
  ].join('\n');
}

describe('pipeline', { concurrency: 1 }, () => {
  test('a pipeline record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-pip-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-pip-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-pip-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-pip-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-pip-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-pip-west-'));
    ...
    assert.equal(unsetOut.error, 'pipeline_root_required');
    ...
    assert.equal(homeOut.error, 'pipeline_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-PIPELINE.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-PIPELINE-PLAN.md'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'pipeline_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-PIPELINE-PLAN.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-PIPELINE-PLAN.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-PIPELINE-BRIEF.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'PIPELINE notes only.');
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-PIPELINE-PLAN.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(stored.markers.PIPELINE, true);
    assert.equal(stored.markers.BOARD, true);
    assert.equal(stored.markers.FORM, true);
    assert.equal(stored.markers.SUBMISSION, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'pipeline', 'pipeline.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west pipeline failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-PIPELINE-PLAN.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'pipeline', 'pipeline.shot')), false);
