...
const CLI = path.join(OPS, 'demigod-heavy-copy-prompt.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/heavy-copy-spec';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'heavy-copy-spec');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-COPY.json',
  '/home/potter/HEAVY-COPY-SPEC.md',
  path.join(OPS, 'DEMIGOD-HEAVY-COPY.json'),
  path.join(OPS, 'HEAVY-COPY-SPEC.md'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-copy-prompt') || name.startsWith('HEAVY-COPY-SPEC')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-COPY.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-COPY-BRIEF.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'COPY notes stay local.',
    'SPEC lists the exact line.',
    'FOOT file stays in the root.',
    'HEAD file stays in the root.',
    'STRING values stay on disk.',
  ].join('\n');
}

describe('heavy copy spec', { concurrency: 1 }, () => {
  test('a copy spec stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-spec-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-spec-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-spec-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-spec-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-spec-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-spec-west-'));
    ...
    assert.equal(unsetOut.error, 'copy_spec_root_required');
    ...
    assert.equal(homeOut.error, 'copy_spec_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-COPY.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-COPY-SPEC.md'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'copy_spec_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-COPY-SPEC.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-COPY-BRIEF.md'));
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-COPY-SPEC.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('SPEC'), true);
    assert.equal(badStored.missing.includes('FOOT'), true);
    assert.equal(badStored.missing.includes('HEAD'), true);
    assert.equal(badStored.missing.includes('STRING'), true);

    plant(east, 'Harbor East keep', goodBody());
    const eastRun = run(east, [], {}, cwdDir);
    assert.equal(eastRun.status, 0, `east spec failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.SPEC, true);
    assert.equal(stored.markers.FOOT, true);
    assert.equal(stored.markers.HEAD, true);
    assert.equal(stored.markers.STRING, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'heavy-copy-spec', 'spec.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west spec failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-COPY-SPEC.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'heavy-copy-spec', 'spec.shot')), false);
