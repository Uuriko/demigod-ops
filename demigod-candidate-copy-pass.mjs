#!/usr/bin/env node
/**
 * Local candidate copy check in an explicit data root.
 * Writes DEMIGOD-CANDIDATE-COPY-PASS.json under DEMIGOD_ROOT. Does not open a browser or send a prompt.
 */
import fs from 'fs';
...
function reportPath() {
  return path.join(dataRoot(), 'DEMIGOD-CANDIDATE-COPY-PASS.json');
}
function shotDir() {
  return path.join(dataRoot(), 'audit-shots', 'candidate-copy');
}
...
function main() {
  ...
  if (!root || path.resolve(root) === '/home/potter' || path.resolve(root) === path.resolve(scriptDir())) {
    refuse('candidate_root_required');
  }
  const footText = readText(root, 'demigod-foot-core.js');
  const notes = readText(root, 'HEAVY-CANDIDATE-COPY-SOURCE.md');
  ...
  const dir = shotDir();
  const shot = path.join(dir, 'copy.shot');
