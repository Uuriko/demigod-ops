...
const CLI = path.join(OPS, 'demigod-heavy-website-prompt.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/website-prompt';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'website-prompt');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-WEBSITE-PROMPT.json',
  '/home/potter/HEAVY-CURSOR-WEBSITE-PROMPT.md',
  '/home/potter/HEAVY-CURSOR-WEBSITE-SENT.txt',
  path.join(OPS, 'DEMIGOD-HEAVY-WEBSITE-PROMPT.json'),
  path.join(OPS, 'HEAVY-CURSOR-WEBSITE-PROMPT.md'),
  path.join(OPS, 'HEAVY-CURSOR-WEBSITE-SENT.txt'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-website-prompt') || name.startsWith('DEMIGOD-HEAVY-WEBSITE-PROMPT') || name.startsWith('HEAVY-CURSOR-WEBSITE')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-WEBSITE-PROMPT.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-CURSOR-WEBSITE-BRIEF.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'WEBSITE notes stay local.',
    'PROMPT stays on disk.',
    'BRIEF stays planted.',
    'CURSOR stays local.',
...
describe('website prompt', { concurrency: 1 }, () => {
  test('a website prompt record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-wpr-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-wpr-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-wpr-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-wpr-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-wpr-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-wpr-west-'));
    ...
    assert.equal(src.includes('readFileSync(SENT'), false);
    ...
    assert.equal(unsetOut.error, 'prompt_root_required');
    ...
    assert.equal(homeOut.error, 'prompt_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-WEBSITE-PROMPT.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-CURSOR-WEBSITE-PROMPT.md'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-CURSOR-WEBSITE-SENT.txt'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'prompt_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-CURSOR-WEBSITE-PROMPT.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-CURSOR-WEBSITE-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-CURSOR-WEBSITE-PROMPT.md')), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-CURSOR-WEBSITE-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-CURSOR-WEBSITE-BRIEF.md'));
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-CURSOR-WEBSITE-PROMPT.md')), false);
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-CURSOR-WEBSITE-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('BRIEF'), true);
    assert.equal(badStored.missing.includes('CURSOR'), true);
    assert.equal(badStored.missing.includes('LOCAL'), true);

    plant(east, 'Harbor East keep', goodBody());
    const eastRun = run(east, [], {}, cwdDir);
    assert.equal(eastRun.status, 0, `east prompt failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.BRIEF, true);
    assert.equal(stored.markers.CURSOR, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'website-prompt', 'prompt.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west prompt failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-CURSOR-WEBSITE-PROMPT.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-CURSOR-WEBSITE-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'website-prompt', 'prompt.shot')), false);
