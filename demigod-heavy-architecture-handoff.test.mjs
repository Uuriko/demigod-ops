...
const CLI = path.join(OPS, 'demigod-heavy-architecture-handoff.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/architecture-handoff';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'architecture-handoff');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-ARCHITECTURE-HANDOFF.json',
  '/home/potter/HEAVY-SITE-ARCHITECTURE-REPLY.md',
  '/home/potter/HEAVY-SITE-ARCHITECTURE-SENT.txt',
  path.join(OPS, 'DEMIGOD-HEAVY-ARCHITECTURE-HANDOFF.json'),
  path.join(OPS, 'HEAVY-SITE-ARCHITECTURE-REPLY.md'),
  path.join(OPS, 'HEAVY-SITE-ARCHITECTURE-SENT.txt'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-architecture-') || name.startsWith('HEAVY-SITE-')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-ARCHITECTURE-HANDOFF.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-SITE-ARCHITECTURE-REPORT.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'CDN foot is the live shell.',
    'The wizard collects the brief.',
    'The partner form stays local.',
    'Legal copy is recorded on disk.',
  ].join('\n');
}

describe('architecture handoff', { concurrency: 1 }, () => {
  test('a handoff stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-handoff-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-handoff-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-handoff-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-handoff-out-'));
    ...
    assert.equal(unsetOut.error, 'handoff_root_required');
    ...
    assert.equal(homeOut.error, 'handoff_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-ARCHITECTURE-HANDOFF.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-SITE-ARCHITECTURE-SENT.txt'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'handoff_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-SITE-ARCHITECTURE-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-SITE-ARCHITECTURE-REPORT.md'));
    const linked = run(leak, [], {}, cwdDir);
    const linkedOut = readJson(linked, 'symlink report');
    ...
    plant(bad, 'Harbor Bad keep', 'A short note with no architecture topics.');
    const badRun = run(bad, [], {}, cwdDir);
    assert.equal(badRun.status, 1, `thin report should fail\n${badRun.stdout}\n${badRun.stderr}`);
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-SITE-ARCHITECTURE-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-SITE-ARCHITECTURE-REPLY.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-SITE-ARCHITECTURE-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-SITE-ARCHITECTURE-REPLY.md')), false);
    ...
    assert.equal(stored.topics.CDN, true);
    assert.equal(stored.topics.wizard, true);
    assert.equal(stored.topics.partner, true);
    assert.equal(stored.topics.legal, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'architecture-handoff', 'handoff.shot'), 'utf8');
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-SITE-ARCHITECTURE-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'architecture-handoff', 'handoff.shot')), false);
