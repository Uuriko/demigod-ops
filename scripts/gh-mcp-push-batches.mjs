#!/usr/bin/env node
/** Write push_files batch JSON files for GitHub MCP. */
import fs from 'fs';
import path from 'path';

const ROOT = '/home/potter/eat-the-sounds';
const OUT = '/tmp/gh-mcp-batches';
const OWNER = 'Uuriko';
const REPO = 'eat-the-sounds';
const BRANCH = 'master';
const MESSAGE = 'Sync canonical game from /home/potter workspace (cohesion3)';
...
batches.forEach((files, i) => {
  const payload = {
    ...
    repo: REPO,
    branch: BRANCH,
    message: `${MESSAGE} [${i + 1}/${batches.length}]`,
    files,
  };
