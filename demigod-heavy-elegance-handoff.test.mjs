...
const CLI = path.join(OPS, 'demigod-heavy-elegance-handoff.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/elegance-handoff';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'elegance-handoff');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-ELEGANCE.json',
  '/home/potter/HEAVY-ELEGANCE-PERF-REPLY.md',
  '/home/potter/HEAVY-ELEGANCE-PERF-SENT.txt',
  path.join(OPS, 'DEMIGOD-HEAVY-ELEGANCE.json'),
  path.join(OPS, 'HEAVY-ELEGANCE-PERF-REPLY.md'),
  path.join(OPS, 'HEAVY-ELEGANCE-PERF-SENT.txt'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-elegance') || name.startsWith('HEAVY-ELEGANCE')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-ELEGANCE.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-ELEGANCE-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'BLOAT notes stay local.',
    'ELEGANCE stays on disk.',
    'PERFORMANCE stays measured locally.',
    'WIZARD stays in the foot.',
    'v61 patch list stays local.',
  ].join('\n');
}

describe('elegance handoff', { concurrency: 1 }, () => {
  test('an elegance check stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-eleg-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-eleg-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-eleg-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-eleg-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-eleg-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-eleg-west-'));
    ...
    assert.equal(unsetOut.error, 'elegance_root_required');
    ...
    assert.equal(homeOut.error, 'elegance_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-ELEGANCE.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-ELEGANCE-PERF-REPLY.md'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-ELEGANCE-PERF-SENT.txt'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'elegance_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-ELEGANCE-PERF-REPLY.md')), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-ELEGANCE-PERF-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-ELEGANCE-SOURCE.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'BLOAT notes only.');
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-ELEGANCE-PERF-REPLY.md')), false);
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-ELEGANCE-PERF-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('ELEGANCE'), true);
    assert.equal(badStored.missing.includes('PERFORMANCE'), true);
    assert.equal(badStored.missing.includes('WIZARD'), true);
    assert.equal(badStored.missing.includes('v61'), true);
    ...
    assert.equal(stored.markers.BLOAT, true);
    assert.equal(stored.markers.ELEGANCE, true);
    assert.equal(stored.markers.PERFORMANCE, true);
    assert.equal(stored.markers.WIZARD, true);
    assert.equal(stored.markers.v61, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'elegance-handoff', 'elegance.shot'), 'utf8');
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-ELEGANCE-PERF-REPLY.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-ELEGANCE-PERF-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'elegance-handoff', 'elegance.shot')), false);
