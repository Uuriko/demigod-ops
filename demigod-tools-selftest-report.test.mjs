...
const CHECKOUT_REPORT = path.join(OPS, 'DEMIGOD-TOOLS-SELFTEST.json');
const BUSY_REPORT = '/tmp/dg-busy/tools-selftest.json';
...
function report(dir) {
  return JSON.parse(fs.readFileSync(path.join(dir, 'DEMIGOD-TOOLS-SELFTEST.json'), 'utf8'));
}

describe('selftest report', { concurrency: 1 }, () => {
  test('a self-test report stays in that data root', async (t) => {
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-selftest-report-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-selftest-report-west-'));
    ...
    assert.equal(fs.existsSync(path.join(east, 'DEMIGOD-TOOLS-SELFTEST.json')), false);
    ...
    assert.equal(eastOut.selftestPath, path.join(east, 'DEMIGOD-TOOLS-SELFTEST.json'));
    assert.equal(eastOut.sent, false);
    assert.equal(eastOut.liveMail, false);
    assert.equal(eastOut.livePublish, false);
    const eastSaved = report(east);
    assert.equal(eastSaved.pass, true);
    assert.equal(eastSaved.path, path.join(east, 'DEMIGOD-TOOLS-SELFTEST.json'));
    assert.equal(eastSaved.livePublish, false);
    assert.equal(eastSaved.results.some((row) => row.name === 'truth.json written' && row.ok === true), true);
    assert.equal(JSON.stringify(eastSaved).includes(west), false);
    const eastBytes = snapshot(path.join(east, 'DEMIGOD-TOOLS-SELFTEST.json'));

    writeRoot(west, 'Harbor West truth', 92);
    const westRun = run(west, ['--truth']);
    const westOut = readJson(westRun, 'west');
    assert.equal(westRun.status, 0);
    assert.equal(westOut.selftestPath, path.join(west, 'DEMIGOD-TOOLS-SELFTEST.json'));
    assert.equal(report(west).path, path.join(west, 'DEMIGOD-TOOLS-SELFTEST.json'));
    assert.equal(report(west).pass, true);
    assert.equal(JSON.stringify(report(west)).includes(east), false);
    assert.deepEqual(snapshot(path.join(east, 'DEMIGOD-TOOLS-SELFTEST.json')), eastBytes);
