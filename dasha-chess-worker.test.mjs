...
response = await room.handleChess(request('101', '/chess/me', null), 'https://www.getdasha.com');
payload = await response.json();
assert.equal(payload.linked, true);
assert.equal(payload.enrolled, true);
assert.equal(payload.rating.rating, 1200, 'a new X-linked identity starts at provisional 1200 Elo');
assert.equal(payload.rating.games, 0);
...
response = await room.handleChess(request(whiteX, '/chess/me', null), 'https://www.getdasha.com');
assert.equal((await response.json()).rating.rating, 1212, 'rated games must persist Elo on the X-linked identity');
response = await room.handleChess(request(null, '/chess/ratings', null), 'https://www.getdasha.com');
assert.equal((await response.json()).ratings[0].rating, 1212, 'the public table must still show Elo after a rated game');
