...
export function simpResultHtml({ id, title, correct, total, sri = LIVE_SIMP_BOARD_SRI }) {
  ...
  const image = `${LOBBY}/simp/card/${id}.png?v=3`;
...
<script>(function(){var s=document.createElement('script');s.src='${LOBBY}/client/simp-board.js';s.integrity='${sri}';s.crossOrigin='anonymous';s.defer=true;document.head.appendChild(s)})();</script>
<script>(function(){var b=document.querySelector('.dasha-share');if(!b)return;b.addEventListener('click',function(){var title=b.getAttribute('data-title')||'';var text=b.getAttribute('data-text')||'';var url=b.getAttribute('data-url')||'';var copy=function(){var fallback=function(){try{var t=document.createElement('textarea');t.value=text;t.setAttribute('readonly','');t.style.cssText='position:fixed;left:-9999px';document.body.appendChild(t);t.select();document.execCommand('copy');t.remove();}catch(e){}};if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(text).catch(fallback);}else fallback();};if(navigator.share){navigator.share({title:title,text:text,url:url}).catch(function(err){if(!err||err.name!=='AbortError')copy();});}else copy();});}());</script>
</body></html>`;
}
