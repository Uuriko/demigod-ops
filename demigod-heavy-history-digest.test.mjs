...
const CLI = path.join(OPS, 'demigod-heavy-history-digest.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/history-digest';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'history-digest');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-HISTORY-DIGEST.json',
  '/home/potter/DEMIGOD-HEAVY-HISTORY-DIGEST.md',
  path.join(OPS, 'DEMIGOD-HEAVY-HISTORY-DIGEST.json'),
  path.join(OPS, 'DEMIGOD-HEAVY-HISTORY-DIGEST.md'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-history-digest') || name.startsWith('DEMIGOD-HEAVY-HISTORY-DIGEST')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-HISTORY-DIGEST.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-HISTORY-DIGEST-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'HISTORY notes stay local.',
    'DIGEST stays on disk.',
    'TOPIC index stays planted.',
    'THREAD summary stays local.',
    'ARTIFACT stays local.',
  ].join('\n');
}

describe('history digest', { concurrency: 1 }, () => {
  test('a digest stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-hdig-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-hdig-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-hdig-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-hdig-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-hdig-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-hdig-west-'));
    ...
    assert.equal(unsetOut.error, 'digest_root_required');
    ...
    assert.equal(homeOut.error, 'digest_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-HISTORY-DIGEST.json'), false);
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-HISTORY-DIGEST.md'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'digest_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'DEMIGOD-HEAVY-HISTORY-DIGEST.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-HISTORY-DIGEST-SOURCE.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'HISTORY notes only.');
    ...
    assert.equal(fs.existsSync(path.join(bad, 'DEMIGOD-HEAVY-HISTORY-DIGEST.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('DIGEST'), true);
    assert.equal(badStored.missing.includes('TOPIC'), true);
    assert.equal(badStored.missing.includes('THREAD'), true);
    assert.equal(badStored.missing.includes('ARTIFACT'), true);

    plant(east, 'Harbor East keep', goodBody());
    const eastRun = run(east, [], {}, cwdDir);
    assert.equal(eastRun.status, 0, `east digest failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.HISTORY, true);
    assert.equal(stored.markers.DIGEST, true);
    assert.equal(stored.markers.TOPIC, true);
    assert.equal(stored.markers.THREAD, true);
    assert.equal(stored.markers.ARTIFACT, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'history-digest', 'digest.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west digest failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'DEMIGOD-HEAVY-HISTORY-DIGEST.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'history-digest', 'digest.shot')), false);
