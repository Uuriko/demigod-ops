...
const CLI = path.join(OPS, 'demigod-heavy-project-state.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/project-state';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'project-state');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-PROJECT-STATE.json',
  '/home/potter/HEAVY-PROJECT-STATE.md',
  '/home/potter/DEMIGOD-HEAVY-PROJECT-CURSOR-PROMPT.md',
  '/home/potter/HEAVY-PROJECT-STATE-SENT.txt',
  path.join(OPS, 'DEMIGOD-HEAVY-PROJECT-STATE.json'),
  path.join(OPS, 'HEAVY-PROJECT-STATE.md'),
  path.join(OPS, 'DEMIGOD-HEAVY-PROJECT-CURSOR-PROMPT.md'),
  path.join(OPS, 'HEAVY-PROJECT-STATE-SENT.txt'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-project-state') || name.startsWith('DEMIGOD-HEAVY-PROJECT-STATE') || name.startsWith('HEAVY-PROJECT-STATE') || name.startsWith('DEMIGOD-HEAVY-PROJECT-CURSOR')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-PROJECT-STATE.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-PROJECT-STATE-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'STATE notes stay local.',
    'PROJECT stays on disk.',
    'BOARD stays planted.',
    'STATUS stays local.',
...
describe('project state', { concurrency: 1 }, () => {
  test('a state record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-pst-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-pst-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-pst-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-pst-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-pst-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-pst-west-'));
    ...
    assert.equal(src.includes('demigod-status-report'), false);
    ...
    assert.equal(unsetOut.error, 'state_root_required');
    ...
    assert.equal(homeOut.error, 'state_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-PROJECT-STATE.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-PROJECT-STATE.md'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-PROJECT-STATE-SENT.txt'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'state_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-PROJECT-STATE.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-PROJECT-STATE-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'DEMIGOD-HEAVY-PROJECT-CURSOR-PROMPT.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-PROJECT-STATE.md')), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-PROJECT-STATE-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(east, 'DEMIGOD-HEAVY-PROJECT-CURSOR-PROMPT.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-PROJECT-STATE-SOURCE.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'STATE notes only.');
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-PROJECT-STATE.md')), false);
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-PROJECT-STATE-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(bad, 'DEMIGOD-HEAVY-PROJECT-CURSOR-PROMPT.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(stored.markers.STATE, true);
    assert.equal(stored.markers.PROJECT, true);
    assert.equal(stored.markers.BOARD, true);
    assert.equal(stored.markers.STATUS, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'project-state', 'state.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west state failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-PROJECT-STATE-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'DEMIGOD-HEAVY-PROJECT-CURSOR-PROMPT.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'project-state', 'state.shot')), false);
