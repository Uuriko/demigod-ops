...
const WAIT_MS = 90000;
console.log(`waiting ${WAIT_MS / 1000}s for Heavy...`);
...
const data = await page.evaluate(() => {
  ...
  return { text: text.slice(-10000), codeBlocks: codes };
});

fs.writeFileSync('/home/potter/HEAVY-OVERWORLD-STORE-FEEDBACK.md', data.text);
if (data.codeBlocks.length) {
  fs.writeFileSync('/home/potter/HEAVY-OVERWORLD-STORE-CODE.js', data.codeBlocks.join('\n\n'));
}
console.log('saved feedback', data.text.length, 'chars, codes:', data.codeBlocks.length);
