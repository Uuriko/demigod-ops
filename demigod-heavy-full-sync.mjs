#!/usr/bin/env node
/**
 * Local sync check in an explicit data root.
 * Writes DEMIGOD-HEAVY-FULL-SYNC.json under DEMIGOD_ROOT. Does not spawn audits or send a prompt.
 */
import fs from 'fs';
...
function reportPath() {
  return path.join(dataRoot(), 'DEMIGOD-HEAVY-FULL-SYNC.json');
}
function shotDir() {
  return path.join(dataRoot(), 'audit-shots', 'full-sync');
}
...
function main() {
  ...
  if (!root || path.resolve(root) === '/home/potter' || path.resolve(root) === path.resolve(scriptDir())) {
    refuse('sync_root_required');
  }
  const footText = readText(root, 'demigod-foot-core.js');
  const notes = readText(root, 'HEAVY-FULL-SYNC-SOURCE.md');
  ...
  const dir = shotDir();
  const shot = path.join(dir, 'sync.shot');
