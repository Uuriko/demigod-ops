#!/usr/bin/env node
/**
 * Local form-fields check in an explicit data root.
 * Writes DEMIGOD-HEAVY-FORM-FIELDS.json under DEMIGOD_ROOT. Does not send a prompt.
 */
import fs from 'fs';
...
const MARKERS = [
  ...
  { name: 'REQUIRED', re: /required/i },
  { name: 'FIELD', re: /field/i },
];
...
function reportPath() {
  return path.join(dataRoot(), 'DEMIGOD-HEAVY-FORM-FIELDS.json');
}
function shotDir() {
  return path.join(dataRoot(), 'audit-shots', 'form-fields');
}
...
function main() {
  ...
  if (!root || path.resolve(root) === '/home/potter' || path.resolve(root) === path.resolve(scriptDir())) {
    refuse('fields_root_required');
  }
  const footText = readText(root, 'demigod-foot-core.js');
  const notes = readText(root, 'HEAVY-FORM-FIELDS-SOURCE.md');
  ...
  const dir = shotDir();
  const shot = path.join(dir, 'fields.shot');
