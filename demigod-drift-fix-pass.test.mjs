...
const CLI = path.join(OPS, 'demigod-drift-fix-pass.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/drift-fix-pass';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'drift-fix-pass');
const SHARED = [
  '/home/potter/DEMIGOD-DRIFT-FIX.json',
  path.join(OPS, 'DEMIGOD-DRIFT-FIX.json'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-drift-fix') || name.startsWith('DEMIGOD-DRIFT-FIX')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-DRIFT-FIX.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-DRIFT-FIX-PASS-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'DRIFT notes stay local.',
    'FIX stays on disk.',
...
describe('drift fix pass', { concurrency: 1 }, () => {
  test('a drift record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-dfp-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-dfp-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-dfp-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-dfp-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-dfp-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-dfp-west-'));
    ...
    assert.equal(unsetOut.error, 'drift_root_required');
    ...
    assert.equal(homeOut.error, 'drift_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-DRIFT-FIX.json'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'drift_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal((live.stdout + live.stderr).includes('TalentLink'), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-DRIFT-FIX-PASS-SOURCE.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'DRIFT notes only.');
    ...
    assert.equal(badStored.missing.includes('FIX'), true);
    ...
    assert.equal(eastRun.status, 0, `east drift failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.DRIFT, true);
    assert.equal(stored.markers.FIX, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'drift-fix-pass', 'drift.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west drift failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'drift-fix-pass', 'drift.shot')), false);
