#!/usr/bin/env node
/**
 * Local perf check in an explicit data root.
 * Writes DEMIGOD-GROK-PERF.json under DEMIGOD_ROOT. Does not snapshot the machine or send a prompt.
 */
import fs from 'fs';
...
const MARKERS = [
  { name: 'PERF', re: /perf/i },
  { name: 'CONFIG', re: /config/i },
  { name: 'MCP', re: /mcp/i },
  { name: 'TAB', re: /tab/i },
  { name: 'METRIC', re: /metric/i },
];
...
function reportPath() {
  return path.join(dataRoot(), 'DEMIGOD-GROK-PERF.json');
}
function shotDir() {
  return path.join(dataRoot(), 'audit-shots', 'grok-perf');
}
...
function main() {
  ...
  if (!root || path.resolve(root) === '/home/potter' || path.resolve(root) === path.resolve(scriptDir())) {
    refuse('perf_root_required');
  }
  const footText = readText(root, 'demigod-foot-core.js');
  const notes = readText(root, 'HEAVY-GROK-PERF-SOURCE.md');
  ...
  const dir = shotDir();
  const shot = path.join(dir, 'perf.shot');
