...
const CLI = path.join(OPS, 'demigod-heavy-session-handoff.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/session-handoff';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'session-handoff');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-NEXT-RESEARCH.json',
  '/home/potter/HEAVY-NEXT-RESEARCH.md',
  '/home/potter/HEAVY-NEXT-RESEARCH-SENT.txt',
  path.join(OPS, 'DEMIGOD-HEAVY-NEXT-RESEARCH.json'),
  path.join(OPS, 'HEAVY-NEXT-RESEARCH.md'),
  path.join(OPS, 'HEAVY-NEXT-RESEARCH-SENT.txt'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-session-handoff') || name.startsWith('DEMIGOD-HEAVY-NEXT-RESEARCH') || name.startsWith('HEAVY-NEXT-RESEARCH')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-NEXT-RESEARCH.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-SESSION-HANDOFF-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'SESSION notes stay local.',
    'HANDOFF stays on disk.',
    'RESEARCH stays planted.',
    'NEXT stays local.',
    'LOCAL check stays local.',
  ].join('\n');
}

describe('session handoff', { concurrency: 1 }, () => {
  test('a session handoff record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-sho-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-sho-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-sho-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-sho-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-sho-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-sho-west-'));
    ...
    assert.equal(unsetOut.error, 'session_root_required');
    ...
    assert.equal(homeOut.error, 'session_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-NEXT-RESEARCH.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-NEXT-RESEARCH.md'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-NEXT-RESEARCH-SENT.txt'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'session_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-NEXT-RESEARCH.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-NEXT-RESEARCH-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-NEXT-RESEARCH.md')), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-NEXT-RESEARCH-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-SESSION-HANDOFF-SOURCE.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'SESSION notes only.');
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-NEXT-RESEARCH.md')), false);
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-NEXT-RESEARCH-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('HANDOFF'), true);
    assert.equal(badStored.missing.includes('RESEARCH'), true);
    ...
    assert.equal(eastRun.status, 0, `east session failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.HANDOFF, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'session-handoff', 'handoff.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west session failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-NEXT-RESEARCH.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-NEXT-RESEARCH-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'session-handoff', 'handoff.shot')), false);
