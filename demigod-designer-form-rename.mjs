#!/usr/bin/env node
/**
 * Local form rename check in an explicit data root.
 * Writes DEMIGOD-DESIGNER-FORM-RENAME.json under DEMIGOD_ROOT. Does not open a browser or send a prompt.
 */
import fs from 'fs';
...
function reportPath() {
  return path.join(dataRoot(), 'DEMIGOD-DESIGNER-FORM-RENAME.json');
}
function shotDir() {
  return path.join(dataRoot(), 'audit-shots', 'designer-form-rename');
}
...
function main() {
  ...
  if (!root || path.resolve(root) === '/home/potter' || path.resolve(root) === path.resolve(scriptDir())) {
    refuse('rename_root_required');
  }
  const footText = readText(root, 'demigod-foot-core.js');
  const notes = readText(root, 'HEAVY-DESIGNER-FORM-RENAME-SOURCE.md');
  ...
  const dir = shotDir();
  const shot = path.join(dir, 'rename.shot');
