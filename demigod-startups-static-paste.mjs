...
async function run(save) {
  ...
  try {
    ...
    if (save && current.text !== expected) {
      ...
      // Prefer visible save control; fall back to evaluate click (automation id can be present but off-viewport).
      const clicked = await page.evaluate(() => {
        const candidates = [
          ...document.querySelectorAll('[data-automation-id=save-page-button]'),
          ...document.querySelectorAll('button'),
        ];
        const button = candidates.find((el) => {
          if (!(el instanceof HTMLElement) || !el.isConnected) return false;
          const label = `${el.getAttribute('data-automation-id') || ''} ${el.getAttribute('aria-label') || ''} ${el.innerText || ''}`;
          if (!/save/i.test(label)) return false;
          const rect = el.getClientRects();
          return rect.length > 0;
        }) || candidates.find((el) => el.getAttribute('data-automation-id') === 'save-page-button');
        if (!button) return false;
        button.scrollIntoView?.({ block: 'center', inline: 'nearest' });
        button.click();
        return true;
      });
      if (!clicked) throw new Error('save-page-button missing or not clickable');
