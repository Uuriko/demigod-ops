...
const CLI = path.join(OPS, 'demigod-heavy-leverage-next.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/leverage-next';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'leverage-next');
const SHARED = [
  '/home/potter/DEMIGOD-LEVERAGE-NEXT.json',
  '/home/potter/HEAVY-LEVERAGE-NEXT.md',
  '/home/potter/DEMIGOD-LEVERAGE-IMPLEMENT-PROMPT.md',
  '/home/potter/HEAVY-LEVERAGE-NEXT-SENT.txt',
  path.join(OPS, 'DEMIGOD-LEVERAGE-NEXT.json'),
  path.join(OPS, 'HEAVY-LEVERAGE-NEXT.md'),
  path.join(OPS, 'DEMIGOD-LEVERAGE-IMPLEMENT-PROMPT.md'),
  path.join(OPS, 'HEAVY-LEVERAGE-NEXT-SENT.txt'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-leverage-next') || name.startsWith('DEMIGOD-LEVERAGE-NEXT') || name.startsWith('HEAVY-LEVERAGE-NEXT') || name.startsWith('DEMIGOD-LEVERAGE-IMPLEMENT')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-LEVERAGE-NEXT.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-LEVERAGE-NEXT-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'LEVERAGE notes stay local.',
    'NEXT stays on disk.',
    'DEMAND stays planted.',
    'PILOT stays local.',
    'PROOF stays local.',
  ].join('\n');
}

describe('leverage next', { concurrency: 1 }, () => {
  test('a leverage record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-lev-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-lev-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-lev-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-lev-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-lev-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-lev-west-'));
    ...
    assert.equal(unsetOut.error, 'leverage_root_required');
    ...
    assert.equal(homeOut.error, 'leverage_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-LEVERAGE-NEXT.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-LEVERAGE-NEXT.md'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-LEVERAGE-NEXT-SENT.txt'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'leverage_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-LEVERAGE-NEXT.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-LEVERAGE-NEXT-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-LEVERAGE-NEXT.md')), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-LEVERAGE-NEXT-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(east, 'DEMIGOD-LEVERAGE-IMPLEMENT-PROMPT.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-LEVERAGE-NEXT-SOURCE.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'LEVERAGE notes only.');
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-LEVERAGE-NEXT.md')), false);
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-LEVERAGE-NEXT-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(bad, 'DEMIGOD-LEVERAGE-IMPLEMENT-PROMPT.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('NEXT'), true);
    assert.equal(badStored.missing.includes('DEMAND'), true);
    assert.equal(badStored.missing.includes('PILOT'), true);
    assert.equal(badStored.missing.includes('PROOF'), true);

    plant(east, 'Harbor East keep', goodBody());
    const eastRun = run(east, [], {}, cwdDir);
    assert.equal(eastRun.status, 0, `east leverage failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.LEVERAGE, true);
    assert.equal(stored.markers.NEXT, true);
    assert.equal(stored.markers.DEMAND, true);
    assert.equal(stored.markers.PILOT, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'leverage-next', 'next.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west leverage failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-LEVERAGE-NEXT.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-LEVERAGE-NEXT-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'DEMIGOD-LEVERAGE-IMPLEMENT-PROMPT.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'leverage-next', 'next.shot')), false);
