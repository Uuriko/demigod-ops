...
const CLI = path.join(OPS, 'demigod-heavy-full-sync.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/full-sync';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'full-sync');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-FULL-SYNC.json',
  '/home/potter/HEAVY-FULL-SYNC.md',
  '/home/potter/HEAVY-FULL-SYNC-SENT.txt',
  '/home/potter/DEMIGOD-FULL-SYNC-PREAUDIT.json',
  '/home/potter/DEMIGOD-HEAVY-FULL-SYNC-PROMPT.md',
  path.join(OPS, 'DEMIGOD-HEAVY-FULL-SYNC.json'),
  path.join(OPS, 'HEAVY-FULL-SYNC.md'),
  path.join(OPS, 'HEAVY-FULL-SYNC-SENT.txt'),
  path.join(OPS, 'DEMIGOD-FULL-SYNC-PREAUDIT.json'),
  path.join(OPS, 'DEMIGOD-HEAVY-FULL-SYNC-PROMPT.md'),
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-full-sync') || name.startsWith('HEAVY-FULL-SYNC') || name.startsWith('DEMIGOD-FULL-SYNC')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-FULL-SYNC.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-FULL-SYNC-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'SYNC notes stay local.',
    'BUILD work stays on disk.',
    'FORM wizard stays planted.',
    'AUDIT verdict stays local.',
    'WEEK plan stays local.',
  ].join('\n');
}

describe('full sync', { concurrency: 1 }, () => {
  test('a sync record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-sync-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-sync-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-sync-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-sync-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-sync-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-sync-west-'));
    ...
    assert.equal(src.includes('demigod-verify-live'), false);
    ...
    assert.equal(unsetOut.error, 'sync_root_required');
    ...
    assert.equal(homeOut.error, 'sync_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-FULL-SYNC.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-FULL-SYNC.md'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-FULL-SYNC-SENT.txt'), false);
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-FULL-SYNC-PREAUDIT.json'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'sync_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-FULL-SYNC.md')), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-FULL-SYNC-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(east, 'DEMIGOD-FULL-SYNC-PREAUDIT.json')), false);
    assert.equal(fs.existsSync(path.join(east, 'DEMIGOD-HEAVY-FULL-SYNC-PROMPT.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-FULL-SYNC-SOURCE.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'SYNC notes only.');
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-FULL-SYNC.md')), false);
    assert.equal(fs.existsSync(path.join(bad, 'DEMIGOD-FULL-SYNC-PREAUDIT.json')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('BUILD'), true);
    assert.equal(badStored.missing.includes('FORM'), true);
    assert.equal(badStored.missing.includes('AUDIT'), true);
    assert.equal(badStored.missing.includes('WEEK'), true);

    plant(east, 'Harbor East keep', goodBody());
    const eastRun = run(east, [], {}, cwdDir);
    assert.equal(eastRun.status, 0, `east sync failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(fs.existsSync(path.join(east, 'DEMIGOD-FULL-SYNC-PREAUDIT.json')), false);
    assert.equal(fs.existsSync(path.join(east, 'DEMIGOD-HEAVY-FULL-SYNC-PROMPT.md')), false);
    assert.equal(fs.existsSync(path.join(east, 'DEMIGOD-FULL-AUDIT.json')), false);
    ...
    assert.equal(stored.markers.SYNC, true);
    assert.equal(stored.markers.BUILD, true);
    assert.equal(stored.markers.FORM, true);
    assert.equal(stored.markers.AUDIT, true);
    assert.equal(stored.markers.WEEK, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'full-sync', 'sync.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west sync failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-FULL-SYNC.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-FULL-SYNC-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'DEMIGOD-FULL-SYNC-PREAUDIT.json')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'full-sync', 'sync.shot')), false);
