...
const CLI = path.join(OPS, 'demigod-cdp-force-latest.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/cdp-force-latest';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'cdp-force-latest');
const SHARED = [
  '/home/potter/DEMIGOD-CDP-FORCE-LATEST.json',
  path.join(OPS, 'DEMIGOD-CDP-FORCE-LATEST.json'),
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-cdp-force') || name.startsWith('DEMIGOD-CDP-FORCE')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-CDP-FORCE-LATEST.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-CDP-FORCE-LATEST-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'FORCE notes stay local.',
    'LATEST stays on disk.',
...
describe('cdp force latest', { concurrency: 1 }, () => {
  test('a force record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-cfl-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-cfl-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-cfl-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-cfl-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-cfl-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-cfl-west-'));
    ...
    assert.equal(src.includes('browserURL'), false);
    ...
    assert.equal(src.includes('catbox'), false);
    ...
    assert.equal(unsetOut.error, 'force_root_required');
    ...
    assert.equal(homeOut.error, 'force_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-CDP-FORCE-LATEST.json'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'force_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    const shots = run(east, ['--shots'], {}, cwdDir);
    const shotsOut = readJson(shots, 'shots');
    assert.equal(shots.status, 1);
    assert.equal(shotsOut.error, 'publish_refused');
    assert.equal(shotsOut.liveFetch, false);
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(cwdDir, 'audit-shots')), false);
    assert.equal((shots.stdout + shots.stderr).includes('https://'), false);

    const cdn = run(east, ['--cdn=local-note'], {}, cwdDir);
    const cdnOut = readJson(cdn, 'cdn');
    assert.equal(cdn.status, 1);
    assert.equal(cdnOut.error, 'publish_refused');
    assert.equal(cdnOut.liveFetch, false);
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal((cdn.stdout + cdn.stderr).includes('trydemigod.com'), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-CDP-FORCE-LATEST-SOURCE.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'FORCE notes only.');
    ...
    assert.equal(stored.markers.FORCE, true);
    assert.equal(stored.markers.LATEST, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'cdp-force-latest', 'force.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west force failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'cdp-force-latest', 'force.shot')), false);
