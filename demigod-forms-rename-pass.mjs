#!/usr/bin/env node
/**
 * Local forms rename check in an explicit data root.
 * Writes DEMIGOD-FORMS-RENAME.json under DEMIGOD_ROOT. Does not open a browser or send a prompt.
 */
import fs from 'fs';
...
function reportPath() {
  return path.join(dataRoot(), 'DEMIGOD-FORMS-RENAME.json');
}
function shotDir() {
  return path.join(dataRoot(), 'audit-shots', 'forms-rename-pass');
}
...
function main() {
  ...
  const notes = readText(root, 'HEAVY-FORMS-RENAME-PASS-SOURCE.md');
