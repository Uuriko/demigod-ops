...
const CLI = path.join(OPS, 'demigod-heavy-partnership-hybrid-c.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/partnership-hybrid';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'partnership-hybrid');
const SHARED = [
  '/home/potter/DEMIGOD-PARTNERSHIP-HYBRID-C.json',
  '/home/potter/HEAVY-PARTNERSHIP-HYBRID-C.md',
  '/home/potter/DEMIGOD-PARTNERSHIP-IMPLEMENT-PROMPT.md',
  '/home/potter/HEAVY-PARTNERSHIP-HYBRID-C-SENT.txt',
  path.join(OPS, 'DEMIGOD-PARTNERSHIP-HYBRID-C.json'),
  path.join(OPS, 'HEAVY-PARTNERSHIP-HYBRID-C.md'),
  path.join(OPS, 'DEMIGOD-PARTNERSHIP-IMPLEMENT-PROMPT.md'),
  path.join(OPS, 'HEAVY-PARTNERSHIP-HYBRID-C-SENT.txt'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-partnership-hybrid') || name.startsWith('DEMIGOD-PARTNERSHIP-HYBRID') || name.startsWith('HEAVY-PARTNERSHIP-HYBRID') || name.startsWith('DEMIGOD-PARTNERSHIP-IMPLEMENT')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-PARTNERSHIP-HYBRID-C.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-PARTNERSHIP-HYBRID-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'PARTNER notes stay local.',
    'HYBRID stays on disk.',
    'CREDIT stays planted.',
    'REFERRAL stays local.',
    'LOCAL check stays local.',
  ].join('\n');
}

describe('partnership hybrid', { concurrency: 1 }, () => {
  test('a partnership record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-hyb-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-hyb-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-hyb-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-hyb-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-hyb-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-hyb-west-'));
    ...
    assert.equal(unsetOut.error, 'hybrid_root_required');
    ...
    assert.equal(homeOut.error, 'hybrid_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-PARTNERSHIP-HYBRID-C.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-PARTNERSHIP-HYBRID-C.md'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-PARTNERSHIP-HYBRID-C-SENT.txt'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'hybrid_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-PARTNERSHIP-HYBRID-C.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-PARTNERSHIP-HYBRID-C-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-PARTNERSHIP-HYBRID-C.md')), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-PARTNERSHIP-HYBRID-C-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(east, 'DEMIGOD-PARTNERSHIP-IMPLEMENT-PROMPT.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-PARTNERSHIP-HYBRID-SOURCE.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'PARTNER notes only.');
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-PARTNERSHIP-HYBRID-C.md')), false);
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-PARTNERSHIP-HYBRID-C-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(bad, 'DEMIGOD-PARTNERSHIP-IMPLEMENT-PROMPT.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('HYBRID'), true);
    assert.equal(badStored.missing.includes('CREDIT'), true);
    assert.equal(badStored.missing.includes('REFERRAL'), true);
    assert.equal(badStored.missing.includes('LOCAL'), true);

    plant(east, 'Harbor East keep', goodBody());
    const eastRun = run(east, [], {}, cwdDir);
    assert.equal(eastRun.status, 0, `east partnership failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.PARTNER, true);
    assert.equal(stored.markers.HYBRID, true);
    assert.equal(stored.markers.CREDIT, true);
    assert.equal(stored.markers.REFERRAL, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'partnership-hybrid', 'hybrid.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west partnership failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-PARTNERSHIP-HYBRID-C.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-PARTNERSHIP-HYBRID-C-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'DEMIGOD-PARTNERSHIP-IMPLEMENT-PROMPT.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'partnership-hybrid', 'hybrid.shot')), false);
