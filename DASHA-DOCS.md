...
- Live drift: home still says `THE CASINO IS OPEN`, exposes `JOIN THE CHAOS`, omits format state from its five seed links, lacks the adjacent public mint-source link, and shares the stale casino metadata/image. Studio v3 has post/story/banner controls, lineage and three-size downloads, but predates the prepared mint/Buy navigation and narrow-screen target cleanup; its social image is still the remote casino raster. Desk still exposes preset buy amounts, buy-pressure/net-buy copy, a referral parameter and a hash-only `Raid` link; its OG title/copy/image are also casino-era.
