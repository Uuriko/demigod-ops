...
const CLI = path.join(OPS, 'demigod-canvas-simplify.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/canvas-simplify';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'canvas-simplify');
const SHARED = [
  '/home/potter/DEMIGOD-CANVAS-SIMPLIFY.json',
  path.join(OPS, 'DEMIGOD-CANVAS-SIMPLIFY.json'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-canvas-simplify') || name.startsWith('DEMIGOD-CANVAS-SIMPLIFY')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-CANVAS-SIMPLIFY.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-CANVAS-SIMPLIFY-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'CANVAS notes stay local.',
    'SIMPLIFY stays on disk.',
    'METHODOLOGY stays planted.',
    'NAV stays local.',
    'LOCAL check stays local.',
  ].join('\n');
}

describe('canvas simplify', { concurrency: 1 }, () => {
  test('a canvas record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-cvs-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-cvs-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-cvs-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-cvs-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-cvs-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-cvs-west-'));
    ...
    assert.equal(unsetOut.error, 'canvas_root_required');
    ...
    assert.equal(homeOut.error, 'canvas_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-CANVAS-SIMPLIFY.json'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'canvas_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(captureOut.livePublish, false);
    assert.equal(fs.existsSync(reportFile(east)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-CANVAS-SIMPLIFY-SOURCE.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'CANVAS notes only.');
    ...
    assert.equal(badStored.missing.includes('SIMPLIFY'), true);
    assert.equal(badStored.missing.includes('METHODOLOGY'), true);
    ...
    assert.equal(eastRun.status, 0, `east canvas failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.SIMPLIFY, true);
    assert.equal(stored.markers.METHODOLOGY, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'canvas-simplify', 'simplify.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west canvas failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'canvas-simplify', 'simplify.shot')), false);
