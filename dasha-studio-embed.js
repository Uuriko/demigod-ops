...
/* Cold open: invite one material change before Share (open→edit lever). */
if (!inbound && !fragmentHasState && !imageOnly) {
  const el = $('status');
  if (el && (!el.textContent || / · ready$/.test(el.textContent) || /try Today or Surprise/i.test(el.textContent))) {
    el.textContent = 'Change one thing — tap Surprise or Today, or rewrite the line';
  }
  ...
  /* Soft spotlight on the line field without forcing the mobile keyboard. */
  const line = $('line');
  if (line && !line.value.trim()) {
    line.setAttribute('aria-description', 'Edit this line or use Surprise / Today above');
  }
}
