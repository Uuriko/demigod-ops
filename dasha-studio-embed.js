...
/* Cold open: invite play without a tutorial when this is a blank session. */
if (!inbound && !fragmentHasState && !imageOnly) {
  const el = $('status');
  if (el && (!el.textContent || / · ready$/.test(el.textContent))) {
    el.textContent = `${craftStatus()} · try Today or Surprise`;
  }
}
