#!/usr/bin/env node
/**
 * Local website prompt check in an explicit data root.
 * Writes DEMIGOD-HEAVY-WEBSITE-PROMPT.json under DEMIGOD_ROOT. Does not open a browser or send a prompt.
 */
import fs from 'fs';
...
function reportPath() {
  return path.join(dataRoot(), 'DEMIGOD-HEAVY-WEBSITE-PROMPT.json');
}
function shotDir() {
  return path.join(dataRoot(), 'audit-shots', 'website-prompt');
}
...
function main() {
  ...
  if (!root || path.resolve(root) === '/home/potter' || path.resolve(root) === path.resolve(scriptDir())) {
    refuse('prompt_root_required');
  }
  const footText = readText(root, 'demigod-foot-core.js');
  const notes = readText(root, 'HEAVY-CURSOR-WEBSITE-BRIEF.md');
