...
const CLI = path.join(OPS, 'demigod-heavy-improve-prompt.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/improve-prompt';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'improve-prompt');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-IMPROVE-PROMPT.json',
  '/home/potter/HEAVY-CURSOR-IMPROVE-PROMPT.md',
  path.join(OPS, 'DEMIGOD-HEAVY-IMPROVE-PROMPT.json'),
  path.join(OPS, 'HEAVY-CURSOR-IMPROVE-PROMPT.md'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-improve-prompt') || name.startsWith('DEMIGOD-HEAVY-IMPROVE-PROMPT') || name.startsWith('HEAVY-CURSOR-IMPROVE-PROMPT')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-IMPROVE-PROMPT.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-CURSOR-IMPROVE-BRIEF.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'IMPROVE notes stay local.',
...
describe('improve prompt', { concurrency: 1 }, () => {
  test('an improve record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-imp-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-imp-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-imp-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-imp-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-imp-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-imp-west-'));
    ...
    assert.equal(unsetOut.error, 'improve_root_required');
    ...
    assert.equal(homeOut.error, 'improve_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-IMPROVE-PROMPT.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-CURSOR-IMPROVE-PROMPT.md'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'improve_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-CURSOR-IMPROVE-PROMPT.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-CURSOR-IMPROVE-BRIEF.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'IMPROVE notes only.');
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-CURSOR-IMPROVE-PROMPT.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(eastRun.status, 0, `east improve failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.IMPROVE, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'improve-prompt', 'prompt.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west improve failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-CURSOR-IMPROVE-PROMPT.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'improve-prompt', 'prompt.shot')), false);
