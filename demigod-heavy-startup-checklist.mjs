#!/usr/bin/env node
/**
 * Local startup checklist in an explicit data root.
 * Writes DEMIGOD-STARTUP-CHECKLIST.json under DEMIGOD_ROOT. Does not open a browser or send a prompt.
 */
import fs from 'fs';
...
const MARKERS = [
  { name: 'STARTUP', re: /startup/i },
  { name: 'CHECKLIST', re: /checklist/i },
  { name: 'SETUP', re: /setup/i },
  { name: 'PLAN', re: /plan/i },
...
function reportPath() {
  return path.join(dataRoot(), 'DEMIGOD-STARTUP-CHECKLIST.json');
}
function shotDir() {
  return path.join(dataRoot(), 'audit-shots', 'startup-checklist');
}
...
function main() {
  ...
  if (!root || path.resolve(root) === '/home/potter' || path.resolve(root) === path.resolve(scriptDir())) {
    refuse('checklist_root_required');
  }
  const footText = readText(root, 'demigod-foot-core.js');
  const notes = readText(root, 'HEAVY-STARTUP-CHECKLIST-SOURCE.md');
  ...
  const dir = shotDir();
  const shot = path.join(dir, 'checklist.shot');
