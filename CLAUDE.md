...
- Website truth comes only from `bin/dg truth`; never copy a release version
  into this file.
- Check tracked work with `bin/dg-bus inbox claude --unread` and `bin/dg-bus status` at task boundaries. Hold the foot lock for foot-core edits.
- Cross-agent protocol: [`AGENT-COMMS.md`](AGENT-COMMS.md). `ask-claude` remains the direct stateless adapter.
