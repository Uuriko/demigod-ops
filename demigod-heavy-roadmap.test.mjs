...
const CLI = path.join(OPS, 'demigod-heavy-roadmap.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/roadmap';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'roadmap');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-ROADMAP.json',
  '/home/potter/HEAVY-CURSOR-ROADMAP.md',
  path.join(OPS, 'DEMIGOD-HEAVY-ROADMAP.json'),
  path.join(OPS, 'HEAVY-CURSOR-ROADMAP.md'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-roadmap') || name.startsWith('DEMIGOD-HEAVY-ROADMAP') || name.startsWith('HEAVY-CURSOR-ROADMAP')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-ROADMAP.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-CURSOR-ROADMAP-BRIEF.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'ROADMAP notes stay local.',
    'FINISH stays on disk.',
    'START stays planted.',
    'PLAN stays local.',
...
describe('roadmap', { concurrency: 1 }, () => {
  test('a roadmap record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-rdm-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-rdm-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-rdm-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-rdm-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-rdm-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-rdm-west-'));
    ...
    assert.equal(unsetOut.error, 'roadmap_root_required');
    ...
    assert.equal(homeOut.error, 'roadmap_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-ROADMAP.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-CURSOR-ROADMAP.md'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'roadmap_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-CURSOR-ROADMAP.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-CURSOR-ROADMAP.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-CURSOR-ROADMAP-BRIEF.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'ROADMAP notes only.');
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-CURSOR-ROADMAP.md')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('FINISH'), true);
    assert.equal(badStored.missing.includes('START'), true);
    ...
    assert.equal(eastRun.status, 0, `east roadmap failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.ROADMAP, true);
    assert.equal(stored.markers.FINISH, true);
    assert.equal(stored.markers.START, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'roadmap', 'roadmap.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west roadmap failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-CURSOR-ROADMAP.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'roadmap', 'roadmap.shot')), false);
