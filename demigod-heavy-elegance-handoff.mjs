#!/usr/bin/env node
/**
 * Local elegance check in an explicit data root.
 * Writes DEMIGOD-HEAVY-ELEGANCE.json under DEMIGOD_ROOT. Does not send a prompt.
 */
import fs from 'fs';
...
const MARKERS = [
  { name: 'BLOAT', re: /bloat/i },
  { name: 'ELEGANCE', re: /elegance/i },
  { name: 'PERFORMANCE', re: /performance/i },
  { name: 'WIZARD', re: /wizard/i },
  { name: 'v61', re: /v61/i },
];
...
function reportPath() {
  return path.join(dataRoot(), 'DEMIGOD-HEAVY-ELEGANCE.json');
}
function shotDir() {
  return path.join(dataRoot(), 'audit-shots', 'elegance-handoff');
}
...
function main() {
  ...
  if (!root || path.resolve(root) === '/home/potter' || path.resolve(root) === path.resolve(scriptDir())) {
    refuse('elegance_root_required');
  }
  const footText = readText(root, 'demigod-foot-core.js');
  const notes = readText(root, 'HEAVY-ELEGANCE-SOURCE.md');
  ...
  const dir = shotDir();
  const shot = path.join(dir, 'elegance.shot');
