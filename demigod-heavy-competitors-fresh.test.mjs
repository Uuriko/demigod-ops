...
const CLI = path.join(OPS, 'demigod-heavy-competitors-fresh.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/competitors-fresh';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'competitors-fresh');
const SHARED = [
  '/home/potter/DEMIGOD-HEAVY-COMPETITORS-HANDOFF.json',
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-competitors-fresh') || name.startsWith('HEAVY-COMPETITORS-FEATURES')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-HEAVY-COMPETITORS-HANDOFF.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-COMPETITORS-LOCAL-RESEARCH.md'), `${phrase}\n${body}\n`);
}
...
function goodBody() {
  return [
    ...
    'Build the proof loop on Monday.',
    'stop the extra site chrome.',
  ].join('\n');
}

describe('competitors fresh', { concurrency: 1 }, () => {
  test('a fresh handoff stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-fresh-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-fresh-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-fresh-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-fresh-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-fresh-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-fresh-west-'));
    ...
    const outsideFile = path.join(outside, 'digest.md');
    fs.writeFileSync(outsideFile, 'Outside keep secret\nCOMPETITOR BACKLOG Monday stop\n');
    ...
    assert.equal(unsetOut.error, 'fresh_root_required');
    ...
    assert.equal(homeOut.error, 'fresh_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-HEAVY-COMPETITORS-HANDOFF.json'), false);
    ...
    assert.equal(checkoutOut.error, 'fresh_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-COMPETITORS-LOCAL-RESEARCH.md'));
    const linked = run(leak, [], {}, cwdDir);
    const linkedOut = readJson(linked, 'symlink digest');
    ...
    assert.equal(badRun.status, 1, `thin digest should fail\n${badRun.stdout}\n${badRun.stderr}`);
    ...
    assert.equal(badStored.missing.includes('BACKLOG'), true);
    assert.equal(badStored.missing.includes('Monday'), true);
    assert.equal(badStored.missing.includes('stop'), true);

    plant(east, 'Harbor East keep', goodBody());
    const eastRun = run(east, [], {}, cwdDir);
    assert.equal(eastRun.status, 0, `east handoff failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.BACKLOG, true);
    assert.equal(stored.markers.Monday, true);
    assert.equal(stored.markers.stop, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'competitors-fresh', 'fresh.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west handoff failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'competitors-fresh', 'fresh.shot')), false);
