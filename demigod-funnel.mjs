...
function cmdHygiene() {
  ...
  // `ok` is false for ANY error-severity flag, and also for the empty/missing drafts dir
  // (which returns ok:false explicitly, so a vacuous `[].every()` green cannot reach here).
  // The old `&& report.checked === 0` meant the only thing that could fail this gate was
  // having nothing to check: 24 drafts with a competitor-addressed draft among them exited 0.
