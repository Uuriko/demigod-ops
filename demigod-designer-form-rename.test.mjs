...
const CLI = path.join(OPS, 'demigod-designer-form-rename.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/designer-form-rename';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'designer-form-rename');
const SHARED = [
  '/home/potter/DEMIGOD-DESIGNER-FORM-RENAME.json',
  path.join(OPS, 'DEMIGOD-DESIGNER-FORM-RENAME.json'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-designer-form') || name.startsWith('DEMIGOD-DESIGNER-FORM')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-DESIGNER-FORM-RENAME.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-DESIGNER-FORM-RENAME-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'FORM notes stay local.',
    'RENAME stays on disk.',
    'STARTUP stays planted.',
    'ENGINEER stays local.',
    'LOCAL check stays local.',
  ].join('\n');
}

describe('designer form rename', { concurrency: 1 }, () => {
  test('a rename record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-dfr-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-dfr-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-dfr-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-dfr-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-dfr-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-dfr-west-'));
    ...
    assert.equal(unsetOut.error, 'rename_root_required');
    ...
    assert.equal(homeOut.error, 'rename_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-DESIGNER-FORM-RENAME.json'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'rename_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    const removed = run(east, ['--delete'], {}, cwdDir);
    const removedOut = readJson(removed, 'delete');
    assert.equal(removed.status, 1);
    assert.equal(removedOut.error, 'publish_refused');
    assert.equal(removedOut.livePublish, false);
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal((removed.stdout + removed.stderr).includes('TalentLink'), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-DESIGNER-FORM-RENAME-SOURCE.md'));
    ...
    assert.equal(badStored.missing.includes('RENAME'), true);
    ...
    assert.equal(eastRun.status, 0, `east rename failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.RENAME, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'designer-form-rename', 'rename.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west rename failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'designer-form-rename', 'rename.shot')), false);
