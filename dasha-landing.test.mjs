...
for (const required of ['$dasha', mint, '/dasha', '/studio', '/lobby', 'Make something', 'Simp board', 'Contribute', 'dasha-simp-board', 'client/simp-board.js', 'jup.ag/swap', 'geckoterminal.com']) assert(html.includes(required), `missing ${required}`);
...
for (const width of [320, 390, 1440]) {
  ...
  assert.equal(await page.$eval('main', el => { const clone=el.cloneNode(true);clone.querySelector('#dasha-simp-board')?.remove();return clone.innerText.trim().split(/\s+/).length < 180; }), true, 'homepage chrome became text-heavy again');
  ...
  if (width === 390) assert.deepEqual(await page.$eval('.navlinks', nav => [...nav.children].map(link => [link.textContent.trim(), getComputedStyle(link).display !== 'none'])), [
    ['Studio', false], ['Lobby', false], ['CA 53ux…pump', false], ['Buy $dasha ↗', true],
