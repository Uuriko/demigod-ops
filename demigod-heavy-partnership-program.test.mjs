...
const CLI = path.join(OPS, 'demigod-heavy-partnership-program.mjs');
const HOME_SHOTS = '/home/potter/audit-shots/partnership-program';
const OPS_SHOTS = path.join(OPS, 'audit-shots', 'partnership-program');
const SHARED = [
  '/home/potter/DEMIGOD-PARTNERSHIP-PROGRAM.json',
  '/home/potter/HEAVY-PARTNERSHIP-PROGRAM.md',
  '/home/potter/HEAVY-PARTNERSHIP-PROGRAM-SENT.txt',
  path.join(OPS, 'DEMIGOD-PARTNERSHIP-PROGRAM.json'),
  path.join(OPS, 'HEAVY-PARTNERSHIP-PROGRAM.md'),
  path.join(OPS, 'HEAVY-PARTNERSHIP-PROGRAM-SENT.txt'),
];
...
function tmpReports() {
  return fs.readdirSync('/tmp').filter((name) => name.startsWith('demigod-heavy-partnership-program') || name.startsWith('DEMIGOD-PARTNERSHIP-PROGRAM') || name.startsWith('HEAVY-PARTNERSHIP-PROGRAM')).sort();
}

function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-PARTNERSHIP-PROGRAM.json');
}

function plant(dir, phrase, body) {
  fs.writeFileSync(path.join(dir, 'demigod-foot-core.js'), `// ${phrase}\ndg-foot-v75-core\n`);
  fs.writeFileSync(path.join(dir, 'HEAVY-PARTNERSHIP-PROGRAM-SOURCE.md'), `${phrase}\n${body}\n`);
}

function goodBody() {
  return [
    'PROGRAM notes stay local.',
    'PARTNER stays on disk.',
    'REFERRAL stays planted.',
    'CREDIT stays local.',
    'LOCAL check stays local.',
  ].join('\n');
}

describe('partnership program', { concurrency: 1 }, () => {
  test('a program record stays in that data root', async (t) => {
    const cwdDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-prg-cwd-'));
    const bad = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-prg-bad-'));
    const leak = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-prg-leak-'));
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-prg-out-'));
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-prg-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-prg-west-'));
    ...
    assert.equal(unsetOut.error, 'program_root_required');
    ...
    assert.equal(homeOut.error, 'program_root_required');
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-PARTNERSHIP-PROGRAM.json'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-PARTNERSHIP-PROGRAM.md'), false);
    assert.equal(fs.existsSync('/home/potter/HEAVY-PARTNERSHIP-PROGRAM-SENT.txt'), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);

    const checkout = run(OPS, [], {}, cwdDir);
    const checkoutOut = readJson(checkout, 'checkout root');
    assert.equal(checkout.status, 1);
    assert.equal(checkoutOut.error, 'program_root_required');
    assert.equal(fs.existsSync(reportFile(OPS)), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-PARTNERSHIP-PROGRAM.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-PARTNERSHIP-PROGRAM-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(fs.existsSync(reportFile(east)), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-PARTNERSHIP-PROGRAM.md')), false);
    assert.equal(fs.existsSync(path.join(east, 'HEAVY-PARTNERSHIP-PROGRAM-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    fs.symlinkSync(outsideFile, path.join(leak, 'HEAVY-PARTNERSHIP-PROGRAM-SOURCE.md'));
    ...
    plant(bad, 'Harbor Bad keep', 'PROGRAM notes only.');
    ...
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-PARTNERSHIP-PROGRAM.md')), false);
    assert.equal(fs.existsSync(path.join(bad, 'HEAVY-PARTNERSHIP-PROGRAM-SENT.txt')), false);
    assert.equal(fs.existsSync(reportFile(cwdDir)), false);
    ...
    assert.equal(badStored.missing.includes('PARTNER'), true);
    ...
    assert.equal(eastRun.status, 0, `east program failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(stored.markers.PROGRAM, true);
    ...
    const eastShot = fs.readFileSync(path.join(east, 'audit-shots', 'partnership-program', 'program.shot'), 'utf8');
    ...
    assert.equal(westRun.status, 0, `west program failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-PARTNERSHIP-PROGRAM.md')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'HEAVY-PARTNERSHIP-PROGRAM-SENT.txt')), false);
    assert.equal(fs.existsSync(path.join(OPS, 'audit-shots', 'partnership-program', 'program.shot')), false);
