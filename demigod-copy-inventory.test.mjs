...
const CLI = path.join(OPS, 'demigod-copy-inventory.mjs');
const HOME_REPORT = '/home/potter/DEMIGOD-STATIC-TEXT-INVENTORY.json';
const CHECKOUT_REPORT = path.join(OPS, 'DEMIGOD-STATIC-TEXT-INVENTORY.json');
...
function plant(dir, phrase) {
  fs.writeFileSync(
    path.join(dir, 'demigod-copy-source.html'),
    `<html><head><meta name="description" content="${phrase}"></head><body><p>${phrase} hiring desk</p></body></html>\n`,
  );
  fs.writeFileSync(
...
function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-STATIC-TEXT-INVENTORY.json');
}

describe('copy inventory', { concurrency: 1 }, () => {
  test('a copy inventory stays in that data root', async (t) => {
    ...
    assert.equal(eastRun.status, 0, `east inventory failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(eastOut.version, '9');
    ...
    assert.equal(stored.runtimeCopy.ctaFounder, 'Harbor East keep');
    assert.equal(stored.staticTextBlocks.includes('Harbor East keep hiring desk'), true);
    ...
    assert.equal(westRun.status, 0, `west inventory failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.equal(westStored.runtimeCopy.ctaFounder, 'Harbor West keep');
