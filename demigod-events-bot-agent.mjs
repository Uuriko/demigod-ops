...
export function isSfLocation(text) {
  ...
  // residual-66: Westwood Park SF — bare "westwood" is LA NON_SF
  if (/\bwestwood\s+park\b/i.test(t)) {
    const rest = t.replace(/\bwestwood\s+park\b/gi, ' ');
...
const AREA_NEAR = {
  ...
  // residual: Holly Park free-list areaNeed missed (draft affinity only; SE → Mission outdoor/kitchen)
  holly: ['holly', 'bernal', 'mission', 'excelsior', 'portola', 'dolores', 'soma'],
  'holly park': ['holly', 'bernal', 'mission', 'excelsior', 'portola', 'dolores', 'soma'],
...
export function scoreFreeVenue(v, { need = '', seats = 0, explain = false } = {}) {
  ...
  if (outdoorAsked && /\b(bayview|visitacion|dogpatch|hunter'?s?\s*point)\b/.test(needL)) {
    if (
      v.id === 'v_soma_parklet' ||
      v.id === 'v_dolores' ||
      /south park|parklet|dolores/.test(blob + ' ' + areaL)
    ) {
      score += 2;
  ...
  if (areaNeed) {
    ...
    else if (/^holly(?:\s+park)?$/.test(tok)) tok = 'holly';
