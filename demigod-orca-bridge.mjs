...
function doctor() {
  ...
  // keep-awake — the systemd unit is the source of truth; .keep-awake.pid is only a
  // cache bin/dg-orca's sync_keep_pid refreshes, so reading it alone false-FAILs
  // whenever the unit is up but unsynced (reboot, unit restart, direct systemd-run).
  try {
    if (spawnSync('systemctl', ['--user', 'is-active', '--quiet', 'demigod-keep-awake.service']).status === 0) {
      ok.push('keep-awake unit active');
    } else {
