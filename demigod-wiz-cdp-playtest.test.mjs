...
const CLI = path.join(OPS, 'demigod-wiz-cdp-playtest.mjs');
const SHARED = [
  '/tmp/audit-wiz-playtest',
  '/home/potter/DEMIGOD-WIZ-PLAYTEST.json',
  path.join(OPS, 'DEMIGOD-WIZ-PLAYTEST.json'),
];
...
function plant(dir, phrase) {
  ...
  fs.writeFileSync(path.join(dir, 'demigod-wiz-source.html'), [
    `<title>${phrase}</title>`,
    '<a>HIRE TALENT</a>',
...
function reportFile(dir) {
  return path.join(dir, 'DEMIGOD-WIZ-PLAYTEST.json');
}

describe('wiz playtest', { concurrency: 1 }, () => {
  test('a wiz playtest stays in that data root', async (t) => {
    const east = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-wiz-east-'));
    const west = fs.mkdtempSync(path.join(os.tmpdir(), 'dg-wiz-west-'));
    ...
    assert.equal(src.includes('/tmp/audit-wiz-playtest'), false);
    ...
    assert.equal(fs.existsSync('/home/potter/DEMIGOD-WIZ-PLAYTEST.json'), false);
    ...
    assert.equal(eastRun.status, 0, `east wiz playtest failed\n${eastRun.stdout}\n${eastRun.stderr}`);
    ...
    assert.equal(eastOut.path.includes('/tmp/audit-wiz-playtest'), false);
    assert.equal(eastOut.shotDir, path.join(east, 'audit-shots', 'wiz-playtest'));
    ...
    assert.equal(eastOut.has90, true);
    assert.equal(eastOut.hasReview, true);
    assert.equal(eastOut.hasThanks, true);
    assert.equal(eastOut.issues, 0);
    assert.equal(eastOut.liveFetch, false);
    assert.equal(eastOut.screenshots.length, 2);
    ...
    for (const file of eastOut.screenshots) {
      ...
      assert.equal(file.includes('/tmp/audit-wiz-playtest'), false);
    ...
    assert.equal(stored.has90, true);
    assert.equal(stored.hasReview, true);
    assert.equal(stored.liveFetch, false);
    assert.equal(JSON.stringify(stored).includes('Harbor West keep'), false);
    const eastBytes = snapshot(path.join(east, 'audit-shots', 'wiz-playtest', '01-modal-open.shot'));

    plant(west, 'Harbor West keep');
    const westRun = run(west);
    assert.equal(westRun.status, 0, `west wiz playtest failed\n${westRun.stdout}\n${westRun.stderr}`);
    ...
    assert.deepEqual(snapshot(path.join(east, 'audit-shots', 'wiz-playtest', '01-modal-open.shot')), eastBytes);
