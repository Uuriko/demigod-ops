#!/usr/bin/env node
/**
 * Local build-research check in an explicit data root.
 * Writes DEMIGOD-GROK-BUILD-RESEARCH.json under DEMIGOD_ROOT. Does not send a prompt.
 */
import fs from 'fs';
...
function reportPath() {
  return path.join(dataRoot(), 'DEMIGOD-GROK-BUILD-RESEARCH.json');
}
function shotDir() {
  return path.join(dataRoot(), 'audit-shots', 'grok-build-research');
}
...
function main() {
  ...
  if (!root || path.resolve(root) === '/home/potter' || path.resolve(root) === path.resolve(scriptDir())) {
    refuse('build_research_root_required');
  }
  const footText = readText(root, 'demigod-foot-core.js');
  const notes = readText(root, 'HEAVY-GROK-BUILD-SOURCE.md');
  ...
  const dir = shotDir();
  const shot = path.join(dir, 'research.shot');
