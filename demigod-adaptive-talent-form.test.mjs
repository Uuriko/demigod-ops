...
test('talent experience adapts to skills while the proof ask stays uniform across paths', () => {
  ...
  assert.equal(context.result[5], 'Strengths / craft / domain *');
  assert.equal(context.result[6], 'What work are you proud of? *');
  assert.equal(context.result[7].q, 'What must your Founding designer be great at?');
  assert.equal(context.result[8].q, 'What must they be great at?');
  ...
  assert.equal((source.match(/steps:\[\['welcome'\],\['full-name'\]/g) || []).length, 1);
});
